...
func ParseDurationFromEnvs(defaultValue, min, max time.Duration, env string, fallbackEnvs ...string) time.Duration {
	envs := append([]string{env}, fallbackEnvs...)

	for _, e := range envs {
		if os.Getenv(e) != "" {
			return ParseDurationFromEnv(e, defaultValue, min, max)
		}
	}
