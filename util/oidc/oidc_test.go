...
func setupAzureIdentity(t *testing.T) {
	...
	tempDir := t.TempDir()
	tokenFilePath := filepath.Join(tempDir, "oidcTokenCache.txt")
...
func Test_azureApp_getFederatedServiceAccountToken(t *testing.T) {
	...
	t.Run("Fetch the oidcTokenCache value from the file", func(t *testing.T) {
	...
	t.Run("Concurrent access to the function when the current oidcTokenCache expires", func(t *testing.T) {
		var wg sync.WaitGroup
		currentExpiryTime := app.expires
...
func TestGetOidcTokenCacheFromJSON(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:                "empty",
			oidcTokenCache:      &OidcTokenCache{},
			expectErrorContains: "empty oidcTokenCache",
		},
		{
			name: "empty id oidcTokenCache",
			oidcTokenCache: &OidcTokenCache{
				Token: &oauth2.Token{},
			},
			expectIdToken: "",
...
func TestClientApp_GetUpdatedOidcTokenFromCache(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectErrorContains: "failed to unmarshal oidc oidcTokenCache refresher: empty oidcTokenCache",
		},
		{
			...
			expectErrorContains: "failed to refresh oidcTokenCache from source: oauth2: token expired and refresh token is not set",
		},
		{
			name:            "cache miss",
			subject:         "",
...
requestedScopes: ["oidc"]`, oidcTestServer.URL, tt.refreshTokenThreshold),
			...
			token, err := app.CheckAndGetRefreshToken(t.Context(), tt.groupClaims, cdSettings.RefreshTokenThreshold())
			if tt.expectErrorContains != "" {
