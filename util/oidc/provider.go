...
func (p *providerImpl) Verify(ctx context.Context, tokenString string, argoSettings *settings.ArgoCDSettings) (*gooidc.IDToken, error) {
	...
	var span trace.Span
	ctx, span = otel.Tracer("auth").Start(ctx, "oidc.providerImpl.Verify")
	defer span.End()
	...
	if !unverifiedHasAudClaim {
		...
		if len(allowedAudiences) == 0 {
			span.RecordError(errors.New("token has an audience claim, but no allowed audiences are configured"))
	...
	if err != nil {
		span.RecordError(err)
		return nil, fmt.Errorf("failed to verify provider oidcTokenCache: %w", err)
	}
...
func (p *providerImpl) verify(ctx context.Context, clientID, tokenString string, skipClientIDCheck bool) (*gooidc.IDToken, error) {
	var span trace.Span
	ctx, span = otel.Tracer("util/oidc").Start(ctx, "oidc.providerImpl.verify")
	defer span.End()
	prov, err := p.provider()
	if err != nil {
		span.RecordError(fmt.Errorf("error getting provider: %w", err))
		return nil, err
	}
	...
	if err != nil {
		...
		if !strings.Contains(err.Error(), "failed to verify signature") {
			...
			span.RecordError(fmt.Errorf("error verifying token: %w", err))
			return nil, err
		}
		newProvider, retryErr := p.newGoOIDCProvider()
		if retryErr != nil {
			span.SetAttributes(attribute.Bool("error", true))
			span.RecordError(fmt.Errorf("hack: error verifying token on retry: %w", err))
		...
		if err != nil {
			span.SetAttributes(attribute.Bool("error", true))
			span.RecordError(fmt.Errorf("hack: error verifying token: %w", err))
			return nil, err
		}
