...
const (
	ScriptTypeHealth          = "health"
	ScriptTypeActionDiscovery = "action-discovery"
	ScriptTypeAction          = "action"
)

// VM Defines a struct that implements the luaVM
type VM struct {
	ResourceOverrides       map[string]appv1.ResourceOverride
	PredefinedScriptsSource func(objKey string, scriptType ScriptType) (string, error)
...
func (vm VM) GetHealthScript(obj *unstructured.Unstructured) (string, error) {
	...
	return vm.getPredefinedLuaScripts(key, ScriptTypeHealth)
}
...
func (vm VM) GetResourceActionDiscovery(obj *unstructured.Unstructured) (string, error) {
	...
	discoveryScript, err := vm.getPredefinedLuaScripts(discoveryKey, ScriptTypeActionDiscovery)
	if err != nil {
		return "", err
	}
...
func (vm VM) GetResourceAction(obj *unstructured.Unstructured, actionName string) (appv1.ResourceActionDefinition, error) {
	...
	actionScript, err := vm.getPredefinedLuaScripts(actionKey, ScriptTypeAction)
	if err != nil {
		return appv1.ResourceActionDefinition{}, err
	}
...
func (vm VM) getPredefinedLuaScripts(objKey string, scriptType ScriptType) (string, error) {
	if vm.PredefinedScriptsSource != nil {
		return vm.PredefinedScriptsSource(objKey, scriptType)
	}
	return "", nil
}
