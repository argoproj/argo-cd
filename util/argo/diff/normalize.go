...
func GetNormalizeConfig(
	lives, configs []*unstructured.Unstructured,
	diffConfig DiffConfig,
) (*ImmutabilityResult, error) {
	result, err := preDiffNormalize(lives, configs, diffConfig)
	if err != nil {
		return nil, err
	}
	...
	for _, live := range result.Lives {
		if live != nil {
			ignoreNormalizerPaths, err := ignoreNormalizer.GetImmutablePaths(live)
			if err != nil {
				return nil, err
			}
			...
			dcr.Lives = append(dcr.Lives, ImmutabilityMapping{
				Entity:         live,
				ImmutablePaths: ignoreNormalizerPaths,
			})
		}
	}
