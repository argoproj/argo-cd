...
func TestValidateRepo(t *testing.T) {
	...
	db.On("GetClusterByUrl", context.Background(), app.Spec.Destination.Server).Return(cluster, nil)
...
func TestValidatePermissions(t *testing.T) {
	...
	t.Run("Application source is not permitted in project", func(t *testing.T) {
		...
		db := &dbmocks.ArgoDB{}
		db.On("GetClusterByUrl", context.Background(), spec.Destination.Server).Return(cluster, nil)
		conditions, err := ValidatePermissions(context.Background(), &spec, &proj, db)
		assert.NoError(t, err)
		assert.Len(t, conditions, 1)
	...
	t.Run("Destination cluster does not exist", func(t *testing.T) {
		...
		db := &dbmocks.ArgoDB{}
		db.On("GetClusterByUrl", context.Background(), spec.Destination.Server).Return(nil, status.Errorf(codes.NotFound, "Cluster does not exist"))
		conditions, err := ValidatePermissions(context.Background(), &spec, &proj, db)
		assert.NoError(t, err)
		assert.Len(t, conditions, 1)
	...
	t.Run("Destination cluster name resolves to valid server", func(t *testing.T) {
		...
		db.On("GetClusterByUrl", context.Background(), "https://127.0.0.1:6443").Return(&cluster, nil)
		conditions, err := ValidatePermissions(context.Background(), &spec, &proj, db)
		assert.NoError(t, err)
