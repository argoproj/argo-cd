...
func TestValidateChartWithoutRevision(t *testing.T) {
	...
	db.EXPECT().GetCluster(mock.Anything, appSpec.Destination.Server).Return(cluster, nil).Maybe()

	conditions, err := ValidatePermissions(t.Context(), appSpec, &argoappv1.AppProject{
		Spec: argoappv1.AppProjectSpec{
			SourceRepos:  []string{"*"},
			Destinations: []argoappv1.ApplicationDestination{{Server: "*", Namespace: "*"}},
		},
