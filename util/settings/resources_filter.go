...
func (rf *ResourcesFilter) checkResourcePresence(apiGroup, kind, cluster string, annotation map[string]string, filteredResources []FilteredResource) bool {

	for _, includedResource := range filteredResources {
		if includedResource.Match(apiGroup, kind, cluster, annotation) {
			return true
		}
	}
...
func (rf *ResourcesFilter) isIncludedResource(apiGroup, kind, cluster string, annotation map[string]string) bool {
	return rf.checkResourcePresence(apiGroup, kind, cluster, annotation, rf.ResourceInclusions)
}

func (rf *ResourcesFilter) isExcludedResource(apiGroup, kind, cluster string, annotation map[string]string) bool {
	return rf.checkResourcePresence(apiGroup, kind, cluster, annotation, rf.getExcludedResources())
}
...
func (rf *ResourcesFilter) IsExcludedResource(apiGroup, kind, cluster string) bool {
	return rf.IsExcludedResourceWithAnnotation(apiGroup, kind, cluster, map[string]string{})
}

func (rf *ResourcesFilter) IsExcludedResourceWithAnnotation(apiGroup, kind, cluster string, annotation map[string]string) bool {
	// if excluded, do not allow
	if rf.isExcludedResource(apiGroup, kind, cluster, annotation) {
		return true
	}

	// if included, do allow
	if rf.isIncludedResource(apiGroup, kind, cluster, annotation) {
		return false
	}
