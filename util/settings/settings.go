...
// isManagedByArgoCD returns true if the secret's TLS cert was
// generated by Argo CD
func isManagedByArgoCD(secret *corev1.Secret) bool {
	return secret.Annotations[annotationTLSManagedByArgoCD] == "true"
}

// saveServerTLSSecret creates or updates the argocd-server-tls secret with
// the given cert and key PEM bytes, and stamps the self-signed annotation
// so Argo CD knows it owns this cert and can renew it
func (mgr *SettingsManager) saveServerTLSSecret(cert, key []byte) error {
	...
	existing.Data[settingServerCertificate] = cert
	existing.Data[settingServerPrivateKey] = key
...
func (mgr *SettingsManager) loadTLSCertificate(settings *ArgoCDSettings, externalSecret *corev1.Secret, argoCDSecret *corev1.Secret) error {
	...
	if externalSecret != nil {
		...
		if err != nil {
			...
			settings.CertificateIsExternal = !isManagedByArgoCD(externalSecret)
		}
	}
	// only fall back to argocd-secret if we don't find a cert at all above
...
func (mgr *SettingsManager) InitializeSettings(insecureModeEnabled bool) (*ArgoCDSettings, error) {
	...
	if cdSettings.Certificate != nil && !cdSettings.CertificateIsExternal && !insecureModeEnabled {
		x509Cert, err := x509.ParseCertificate(cdSettings.Certificate.Certificate[0])
		if err == nil && time.Now().After(x509Cert.NotAfter) {
			log.Info("Argo CD-managed TLS certificate has expired, regenerating")
			cdSettings.Certificate = nil
		}
	}
	if cdSettings.Certificate == nil && !insecureModeEnabled {
		...
		cdSettings.CertificateIsExternal = false
