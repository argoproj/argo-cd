...
func TestV1SecretsTrim(t *testing.T) {
	kubeClient := fake.NewSimpleClientset(
		&v1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{
				Name:      common.ArgoCDConfigMapName,
		...
		&v1.Secret{
			...
			Data: map[string][]byte{
				"webhook.gitlab.secret":          []byte("abc123\n\r"),
				"webhook.github.secret":          []byte("abc123\n\r"),
				"webhook.bitbucket.uuid":         []byte("abc123\n\r"),
				"webhook.bitbucketserver.secret": []byte("abc123\n\r"),
				"webhook.gogs.secret":            []byte("abc123\n\r"),
				"server.secretkey":               nil,
			},
		},
	)
	...
	settings, err := settingsManager.GetSettings()
	assert.NoError(t, err)
	assert.Equal(t, "abc123", settings.WebhookGitHubSecret)
	assert.Equal(t, "abc123", settings.WebhookGitLabSecret)
	assert.Equal(t, "abc123", settings.WebhookBitbucketServerSecret)
	assert.Equal(t, "abc123", settings.WebhookBitbucketUUID)
	assert.Equal(t, "abc123", settings.WebhookGogsSecret)
