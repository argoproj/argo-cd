...
func TestValidateArgoCDServerAddress(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			server:  "https://",
			wantErr: `server address "https://" must be a host and optional port without http:// or https://`,
		},
		{
			name:    "HTTPS URL",
		...
		{
			name:    "path without a host",
			server:  "/argocd",
			wantErr: `server address "/argocd" must be a host and optional port`,
		},
		{
			name:    "trailing slash",
			server:  "localhost:8080/",
			wantErr: `server address "localhost:8080/" must not include a path; use "localhost:8080" instead`,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			err := ValidateArgoCDServerAddress(test.server)
			if test.wantErr == "" {
				require.NoError(t, err)
				return
			}
