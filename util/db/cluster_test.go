...
func TestGetClusterByServerAndName(t *testing.T) {
	...
	t.Run("Valid external cluster", func(t *testing.T) {
		...
		cluster, err := db.GetClusterByServerAndName(t.Context(), string(secretForServerWithExternalClusterAddr.Data["server"]),
			string(secretForServerWithExternalClusterAddr.Data["name"]))
		require.NoError(t, err)
	...
	t.Run("invalid cluster", func(t *testing.T) {
		...
		_, err := db.GetClusterByServerAndName(t.Context(), "https://mycluster-does-not-exist", "mycluster-does-not-exist")
		require.Error(t, err)
	...
	t.Run("in-cluster not configured", func(t *testing.T) {
		...
		cluster, err := db.GetClusterByServerAndName(t.Context(), v1alpha1.KubernetesInternalAPIServerAddr, "in-cluster")
		require.NoError(t, err)
	...
	t.Run("in-cluster disabled", func(t *testing.T) {
		...
		_, err := db.GetClusterByServerAndName(t.Context(), v1alpha1.KubernetesInternalAPIServerAddr, "in-cluster")
		require.Error(t, err)
		status, ok := status.FromError(err)
		assert.True(t, ok)
		assert.Equal(t, codes.NotFound, status.Code())
	...
	t.Run("in-cluster configured", func(t *testing.T) {
		...
		cluster, err := db.GetClusterByServerAndName(t.Context(), v1alpha1.KubernetesInternalAPIServerAddr, "in-cluster-renamed")
		require.NoError(t, err)
		assert.Equal(t, v1alpha1.KubernetesInternalAPIServerAddr, cluster.Server)
