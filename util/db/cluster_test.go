...
func TestGetClusterServersByName(t *testing.T) {
	...
	t.Run("returns the server name", func(t *testing.T) {
		...
		db := NewDB(fakeNamespace, settings.NewSettingsManager(kubeclientset, fakeNamespace), kubeclientset)
...
func TestClusterRaceConditionClusterSecrets(t *testing.T) {
	...
	ctx := t.Context()
	settingsManager := settings.NewSettingsManager(kubeClient, "default")
