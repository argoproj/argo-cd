...
func Test_ValidatePGPKey(t *testing.T) {
	...
	{
		key, err := validatePGPKey(testdata.Github_asc, common.DefaultExecTimeout)
	...
	// Bad case - Garbage
	{
		key, err := validatePGPKey(testdata.Garbage_asc, common.DefaultExecTimeout)
		assert.Error(t, err)
		assert.Nil(t, key)
	}
	// Bad case - more than one key
	{
		key, err := validatePGPKey(testdata.Multi_asc, common.DefaultExecTimeout)
		assert.Error(t, err)
		assert.Nil(t, key)
	}
}
...
func Test_ListConfiguredGPGPublicKeys(t *testing.T) {
	// Good case. Single key in input, right mapping to Key ID in CM
	{
		...
		keys, err := db.ListConfiguredGPGPublicKeys(context.Background(), common.DefaultExecTimeout)
		assert.NoError(t, err)
		assert.Len(t, keys, 1)
	}
...
func Test_AddGPGPublicKey(t *testing.T) {
	// Good case
	{
		...
		// Key should be added
		new, skipped, err := db.AddGPGPublicKey(context.Background(), testdata.Github_asc, common.DefaultExecTimeout)
		assert.NoError(t, err)
		assert.Len(t, new, 1)
		...
		new, skipped, err = db.AddGPGPublicKey(context.Background(), testdata.Multi_asc, common.DefaultExecTimeout)
		assert.NoError(t, err)
		assert.Len(t, new, 2)
		assert.Len(t, skipped, 0)
		cm, err = settings.GetConfigMapByName(common.ArgoCDGPGKeysConfigMapName)
		...
		new, skipped, err = db.AddGPGPublicKey(context.Background(), testdata.Garbage_asc, common.DefaultExecTimeout)
		assert.Error(t, err)
		assert.Nil(t, new)
...
func Test_DeleteGPGPublicKey(t *testing.T) {
	...
	// Good case
	{
		...
		n, err := db.ListConfiguredGPGPublicKeys(context.Background(), common.DefaultExecTimeout)
		assert.NoError(t, err)
		assert.Len(t, n, 1)
