...
func NewRepo(url string, creds git.Creds, insecure, enableLfs bool, fetchRefspecs []string, disco func(root string) (map[string]string, error), reporter metrics.Reporter) (repo.Repo, error) {
	workDir, err := repo.WorkDir(url)
	if err != nil {
		return nil, err
	}
	client, err := git.NewFactory().NewClient(url, workDir, creds, insecure, enableLfs, reporter, fetchRefspecs)
	if err != nil {
		return nil, err
	}
