...
func TestTLSCertificateLimit(t *testing.T) {
	...
	var dataSb552 strings.Builder
	for i := 0; i < CertificateMaxEntriesPerStream+1; i++ {
		dataSb552.WriteString(TestTLSValidSingleCert)
	}
	data += dataSb552.String()
...
func TestSSHKnownHostsLimit(t *testing.T) {
	...
	// Append one more than the max allowed
	var dataSb564 strings.Builder
	for i := 0; i < CertificateMaxEntriesPerStream+1; i++ {
		dataSb564.WriteString(entry)
	}
	data += dataSb564.String()
