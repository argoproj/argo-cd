...
func (c *twoLevelClient) Rename(ctx context.Context, oldKey string, newKey string, expiration time.Duration) error {
	err := c.inMemoryCache.Rename(ctx, oldKey, newKey, expiration)
	if err != nil {
		log.Warnf("Failed to move key '%s' in in-memory cache: %v", oldKey, err)
	}
	return c.externalCache.Rename(ctx, oldKey, newKey, expiration)
}
...
func (c *twoLevelClient) Set(ctx context.Context, item *Item) error {
	...
	err = c.inMemoryCache.Set(ctx, item)
	if err != nil {
		log.Warnf("Failed to save key '%s' in in-memory cache: %v", item.Key, err)
	}
	return c.externalCache.Set(ctx, item)
}
...
func (c *twoLevelClient) Get(ctx context.Context, key string, obj any) error {
	err := c.inMemoryCache.Get(ctx, key, obj)
	if err == nil {
		return nil
	}

	err = c.externalCache.Get(ctx, key, obj)
	if err == nil {
		_ = c.inMemoryCache.Set(ctx, &Item{Key: key, Object: obj})
	}
	return err
}

// Delete deletes cache for given key in both in-memory and external cache.
func (c *twoLevelClient) Delete(ctx context.Context, key string) error {
	err := c.inMemoryCache.Delete(ctx, key)
	if err != nil {
		return err
	}
	return c.externalCache.Delete(ctx, key)
}
...
func (c *twoLevelClient) NotifyUpdated(ctx context.Context, key string) error {
	return c.externalCache.NotifyUpdated(ctx, key)
}
