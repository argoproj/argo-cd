...
func untar(dstPath string, r io.Reader, preserveFileMode bool) error {
	...
	for {
		...
		case tar.TypeSymlink:
			...
			// When dstPath is absolute, realLinkTarget is absolute as well
			relativeLinkTarget, err := filepath.Rel(filepath.Dir(target), realLinkTarget)
			if err != nil {
				return fmt.Errorf("error relativizing link target: %w", err)
			}

			err = os.Symlink(relativeLinkTarget, target)
			if err != nil {
