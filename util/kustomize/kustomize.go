...
func NewKustomizeApp(path string, creds git.Creds, fromRepo string, binaryPath string, execTimeout time.Duration) Kustomize {
	return &kustomize{
		path:        path,
		creds:       creds,
		repo:        fromRepo,
...
type kustomize struct {
	...
	binaryPath string
	// execTimeout determines how long each external binary call make take before timing out.
...
func mapToEditAddArgs(val map[string]string, execTimeout time.Duration) []string {
	var args []string
	if getSemverSafe(execTimeout).LessThan(semver.MustParse("v3.8.5")) {
		arg := ""
...
func (k *kustomize) Build(opts *v1alpha1.ApplicationSourceKustomize, kustomizeOptions *v1alpha1.KustomizeOptions, envVars *v1alpha1.Env) ([]*unstructured.Unstructured, []Image, error) {
	...
	if opts != nil {
		if opts.NamePrefix != "" {
			...
			cmd.Dir = k.path
			_, err := executil.Run(cmd, k.execTimeout)
			if err != nil {
				return nil, nil, err
			}
		}
		...
		if len(opts.CommonLabels) > 0 {
			...
			cmd := exec.Command(k.getBinaryPath(), append(args, mapToEditAddArgs(opts.CommonLabels, k.execTimeout)...)...)
			cmd.Dir = k.path
			_, err := executil.Run(cmd, k.execTimeout)
			if err != nil {
				return nil, nil, err
		...
		if len(opts.CommonAnnotations) > 0 {
			...
			cmd := exec.Command(k.getBinaryPath(), append(args, mapToEditAddArgs(opts.CommonAnnotations, k.execTimeout)...)...)
			cmd.Dir = k.path
			_, err := executil.Run(cmd, k.execTimeout)
			if err != nil {
				return nil, nil, err
	...
	out, err := executil.Run(cmd, k.execTimeout)
	if err != nil {
		return nil, nil, err
	}
...
func getSemver(execTimeout time.Duration) (*semver.Version, error) {
	verStr, err := Version(true, execTimeout)
	if err != nil {
		return nil, err
	}
...
func getSemverSafe(execTimeout time.Duration) *semver.Version {
	if semVer == nil {
		semVerLock.Lock()
		defer semVerLock.Unlock()

		if ver, err := getSemver(execTimeout); err != nil {
...
func Version(shortForm bool, execTimeout time.Duration) (string, error) {
	...
	version, err := executil.Run(cmd, execTimeout)
	if err != nil {
