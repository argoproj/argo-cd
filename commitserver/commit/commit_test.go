...
func Test_CommitHydratedManifests(t *testing.T) {
	...
	t.Run("cross-repo hydration uses dry source repo URL in README and hydrator metadata", func(t *testing.T) {
		...
		// The commit server removes the working directory as soon as CommitAndPush returns, so the manifests
		// (including the README and hydrator.metadata files under test) must be inspected from within this
		// callback, before the deferred cleanup in handleCommitRequest runs.
		var topMetadata hydrator.HydratorCommitMetadata
		var readmeContents string
		mockGitClient.EXPECT().CommitAndPush(mock.Anything, "main", "test commit message").
