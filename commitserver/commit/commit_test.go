...
func Test_CommitHydratedManifests(t *testing.T) {
	...
	t.Run("failed to create git client", func(t *testing.T) {
		...
		mockRepoClientFactory.On("NewClient", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil, assert.AnError).Once()
	...
	t.Run("happy path", func(t *testing.T) {
		...
		mockGitClient.On("CheckoutOrOrphan", mock.Anything, "env/test", false).Return("", nil).Once()
		mockGitClient.On("CheckoutOrNew", mock.Anything, "main", "env/test", false).Return("", nil).Once()
		mockGitClient.On("RemoveContents").Return("", nil).Once()
		mockGitClient.On("CommitAndPush", mock.Anything, "main", "test commit message").Return("", nil).Once()
		mockGitClient.On("CommitSHA").Return("it-worked!", nil).Once()
		mockRepoClientFactory.On("NewClient", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(mockGitClient, nil).Once()
