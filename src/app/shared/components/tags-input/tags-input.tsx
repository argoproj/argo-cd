...
export class TagsInput extends React.Component<TagsInputProps, { tags: string[]; input: string; focused: boolean }> {
    ...
    public render() {
        return (
            <div className={classNames('tags-input argo-field', { 'tags-input--focused': this.state.focused || !!this.state.input })}
                    onClick={() => this.inputEl && this.inputEl.focus()}>
                {this.state.tags.map((tag, i) => (
                    <span className='tags-input__tag' key={tag}>{tag} <i className='fa fa-times' onClick={(e) => {
