...
func TestGetUpdatedClusterInfo_Generation(t *testing.T) {
	const fakeNamespace = "fake-ns"
	const clusterServer = "https://prod.example.com"
	...
	t.Run("sets generation from HashIdentity", func(t *testing.T) {
		kubeclientset := fake.NewClientset(emptyArgoCDConfigMap, argoCDSecret)
		settingsManager := settings.NewSettingsManager(t.Context(), kubeclientset, fakeNamespace)
		argoDB := db.NewDB(fakeNamespace, settingsManager, kubeclientset)
	...
	t.Run("different clusters produce different generations", func(t *testing.T) {
		...
		info1 := updater.getUpdatedClusterInfo(t.Context(), nil, cluster1, nil, metav1.Now())
		info2 := updater.getUpdatedClusterInfo(t.Context(), nil, cluster2, nil, metav1.Now())

		assert.NotEqual(t, info1.Generation, info2.Generation, "different clusters should have different generations")
	})

	t.Run("same cluster produces consistent generation", func(t *testing.T) {
		kubeclientset := fake.NewClientset(emptyArgoCDConfigMap, argoCDSecret)
		settingsManager := settings.NewSettingsManager(t.Context(), kubeclientset, fakeNamespace)
		argoDB := db.NewDB(fakeNamespace, settingsManager, kubeclientset)
		...
		info1 := updater.getUpdatedClusterInfo(t.Context(), nil, cluster, nil, metav1.Now())
		info2 := updater.getUpdatedClusterInfo(t.Context(), nil, cluster, nil, metav1.Now())

		assert.Equal(t, info1.Generation, info2.Generation, "same cluster should produce consistent generation")
	})
}
