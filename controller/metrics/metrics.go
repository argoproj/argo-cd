...
func (c *appCollector) collectApps(ch chan<- prometheus.Metric, app *argoappv1.Application) {
	...
	addGauge(descAppInfo, 1, strconv.FormatBool(autoSyncEnabled), git.NormalizeGitURL(app.Spec.GetSource().RepoURL), app.Spec.Destination.Server, app.Spec.Destination.Namespace, string(syncStatus), string(healthStatus), operation, strconv.FormatBool(isDryRun))
