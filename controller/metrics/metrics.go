...
var (
	...
	descAppInfo = prometheus.NewDesc(
		...
		append(descAppDefaultLabels, "autosync_enabled", "repo", "dest_server", "dest_namespace", "sync_status", "health_status", "operation", "target_revision"),
		nil,
	)
...
func (c *appCollector) collectApps(ch chan<- prometheus.Metric, app *argoappv1.Application) {
	...
	targetRevision := app.Status.Sync.ComparedTo.Source.TargetRevision
	addGauge(descAppInfo, 1, strconv.FormatBool(autoSyncEnabled), git.NormalizeGitURL(app.Spec.GetSource().RepoURL), app.Spec.Destination.Server, app.Spec.Destination.Namespace, string(syncStatus), string(healthStatus), operation, targetRevision)
