...
func TestHydrationUpdateIsRelevant(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		name     string
		mutate   func(newApp *v1alpha1.Application)
		expected bool
	}{
		{
			name:     "resync (same ResourceVersion) is relevant",
			mutate:   func(_ *v1alpha1.Application) {},
			expected: true,
		},
		{
			name: "spec change (Generation bump) is relevant",
			mutate: func(newApp *v1alpha1.Application) {
				newApp.ResourceVersion = "101"
				newApp.Generation = 2
			},
			expected: true,
		},
		{
			name: "annotation change (e.g. refresh/hydrate request) is relevant",
			mutate: func(newApp *v1alpha1.Application) {
				newApp.ResourceVersion = "101"
				newApp.Annotations = map[string]string{"foo": "bar", v1alpha1.AnnotationKeyRefresh: string(v1alpha1.RefreshTypeNormal)}
			},
			expected: true,
		},
		{
			name: "status-only update is not relevant",
			mutate: func(newApp *v1alpha1.Application) {
				newApp.ResourceVersion = "101"
				newApp.Status.Sync.Status = v1alpha1.SyncStatusCodeSynced
			},
			expected: false,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.mutate(newApp)
			assert.Equal(t, tc.expected, hydrationUpdateIsRelevant(oldApp, newApp))
		})
	}
}
