...
// intersectMapWithImmutablePaths will recursively deep-merge valueMap into templateMap, overwriting array items
// in-place. It leaves the paths configured in the ImmutabilityMapping untouched.
func intersectMapWithImmutablePaths(
	templateMap, valueMap map[string]interface{},
	mapping diff.ImmutabilityMapping,
					...
					if anyMatch(append(currentPath, idx), mapping.ImmutablePaths) {
						...
						if tSliceValueMap, ok := innerTSliceValue.(map[string]interface{}); ok {
							if vSliceValueMap, ok := innerVSliceValue.(map[string]interface{}); ok {
								item := intersectMapWithImmutablePaths(
									tSliceValueMap,
									vSliceValueMap,
									mapping,
									append(currentPath, k, idx),
								)
