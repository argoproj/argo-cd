...
func TestCompareAppStateEmpty(t *testing.T) {
	...
	compRes, _ := ctrl.appStateManager.CompareAppState(app, &defaultProj, revisions, sources, false, false, nil, false)
...
func TestAppRevisionsSingleSource(t *testing.T) {
	...
	revisions := make([]string, 0)
	revisions = append(revisions, "")
	compRes, _ := ctrl.appStateManager.CompareAppState(app, &defaultProj, revisions, app.Spec.GetSources(), false, false, nil, app.Spec.HasMultipleSources())
	assert.NotNil(t, compRes)
	assert.NotNil(t, compRes.syncStatus)
...
func TestCompareAppStateManagedNamespaceMetadataWithLiveNsDoesNotGetPruned(t *testing.T) {
	...
	ctrl := newFakeController(&data)
	compRes, _ := ctrl.appStateManager.CompareAppState(app, &defaultProj, []string{}, app.Spec.Sources, false, false, nil, false)
...
func TestSignedResponseSignatureRequired(t *testing.T) {
	...
	// We have a good signature response, valid key, and signing is required - sync!
	{
		...
		revisions := make([]string, 0)
		revisions = append(revisions, "")
		compRes, _ := ctrl.appStateManager.CompareAppState(app, &signedProj, revisions, sources, false, false, nil, false)
		assert.NotNil(t, compRes)
		assert.NotNil(t, compRes.syncStatus)
		assert.Equal(t, argoappv1.SyncStatusCodeSynced, compRes.syncStatus.Status)
		assert.Len(t, compRes.resources, 0)
	...
	// We have a good signature and signing is required, but key is not allowed - do not sync
	{
		...
		revisions := make([]string, 0)
		revisions = append(revisions, "abc123")
		compRes, _ := ctrl.appStateManager.CompareAppState(app, &testProj, revisions, sources, false, false, nil, false)
		assert.NotNil(t, compRes)
		assert.NotNil(t, compRes.syncStatus)
		assert.Equal(t, argoappv1.SyncStatusCodeSynced, compRes.syncStatus.Status)
		assert.Len(t, compRes.resources, 0)
	...
	// Signature required and local manifests supplied - do not sync
	{
		...
		revisions := make([]string, 0)
		revisions = append(revisions, "abc123")
		compRes, _ := ctrl.appStateManager.CompareAppState(app, &signedProj, revisions, sources, false, false, localManifests, false)
		assert.NotNil(t, compRes)
		assert.NotNil(t, compRes.syncStatus)
		assert.Equal(t, argoappv1.SyncStatusCodeUnknown, compRes.syncStatus.Status)
		assert.Len(t, compRes.resources, 0)
