...
func (m *appStateManager) fetchManifests(cmp *appStateCmp, revisions []string, sources []v1alpha1.ApplicationSource, noRevisionCache bool, localManifests []string, rollback bool) (bool, error) {
	var err error
	...
	if len(localManifests) == 0 {
		...
		cmp.targetObjs, cmp.manifestInfos, revisionUpdated, err = m.GetRepoObjs(
			cmp.app, sources, cmp.cmpSettings.appLabelKey, revisions, cmp.noCache, noRevisionCache, cmp.verifySignature, cmp.project, rollback, true,
		)
		if err != nil {
