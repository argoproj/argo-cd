...
func (ctrl *ApplicationController) executePostDeleteHooks(ctx context.Context, app *v1alpha1.Application, proj *v1alpha1.AppProject, liveObjs map[kube.ResourceKey]*unstructured.Unstructured, config *rest.Config, logCtx *log.Entry) (bool, error) {
	...
	targets, _, _, err := ctrl.appStateManager.GetRepoObjs(ctx, app, app.Spec.GetSources(), appLabelKey, revisions, false, false, false, proj, false, true)
	if err != nil {
		return false, err
	}
	...
	for _, obj := range runningHooks {
		hookHealth, err := health.GetResourceHealthContext(ctx, obj, healthOverrides)
		if err != nil {
			return false, err
		}
		if hookHealth == nil {
