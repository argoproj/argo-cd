...
func TestClusterTaintManagerDataIntegrity(t *testing.T) {
	...
	gvks := manager.getTaintedGVKs(server)
	assert.Len(t, gvks, numWorkers*numGVKsPerWorker, "Should have all GVKs")
...
func TestClusterTaintManagerIsolation(t *testing.T) {
	...
	assert.Len(t, gvks1, 100, "Server1 should have 100 GVKs")
	assert.Len(t, gvks2, 100, "Server2 should have 100 GVKs")
	...
	assert.Len(t, manager.getTaintedGVKs(server2), 100, "Server2 should still have all GVKs")
}
