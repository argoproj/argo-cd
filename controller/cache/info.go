...
func populatePodInfo(un *unstructured.Unstructured, res *ResourceInfo) {
	...
	if reason != "Completed" && reason != "Error" {
		res.Info = append(res.Info, v1alpha1.InfoItem{Name: common.PodRequestsCPU, Value: fmt.Sprintf("%d", CPUReq.MilliValue())})    //nolint:perfsprint
		res.Info = append(res.Info, v1alpha1.InfoItem{Name: common.PodRequestsMEM, Value: fmt.Sprintf("%d", MemoryReq.MilliValue())}) //nolint:perfsprint
	}
