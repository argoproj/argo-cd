...
if obj.status ~= nil then
  if obj.status.availableNodes ~= nil then
    ...
    if obj.status.availableNodes < sum then
      ...
      hs.message = "The desired amount of nodes is " .. sum .. " and the availableNodes are " .. obj.status.availableNodes
      return hs
      ...
      if obj.status.phase ~= nil and obj.status.health ~= nil then
        if obj.status.phase == "Ready" then
          ...
          elseif obj.status.health == "yellow" then
            hs.status = "Degraded"
            hs.message = "Elasticsearch Cluster status is Yellow. Check the status of your ES indices, shards and replicas"
            return hs
          elseif obj.status.health == "red" then
            hs.status = "Degraded"
            hs.message = "Elasticsearch Cluster status is Red. Check the status of your ES indices, shards and replicas"
            return hs
          end
