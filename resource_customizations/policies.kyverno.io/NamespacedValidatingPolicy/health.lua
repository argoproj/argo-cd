-- NamespacedValidatingPolicy is a namespace-scoped resource that defines CEL-based
-- validation rules applied to Kubernetes resources during admission via a generated
-- ValidatingAdmissionPolicy or a Kyverno webhook, scoped to a specific namespace.
--
-- Documentation:
--   Policy types overview: https://kyverno.io/docs/policy-types/validating-policy/
--
-- Condition types and reasons are defined in:
--   https://github.com/kyverno/kyverno/tree/main/config/crds/policies.kyverno.io/policies.kyverno.io_namespacedvalidatingpolicies.yaml
--
-- NamespacedValidatingPolicy exposes a conditionStatus with a ready boolean and
  ...
  if obj.metadata.generation ~= nil and cs.conditions ~= nil then
    for _, condition in ipairs(cs.conditions) do
      if condition.observedGeneration ~= nil and condition.observedGeneration < obj.metadata.generation then
        ...
        hs.message = "Waiting for NamespacedValidatingPolicy status to reflect latest generation"
        return hs
      end
    end
  end
...
hs.status = "Progressing"
hs.message = "Waiting for NamespacedValidatingPolicy status"
return hs
