-- MutatingPolicy is a cluster-scoped resource that defines CEL-based mutation
-- rules applied to Kubernetes resources during admission via a generated
-- MutatingAdmissionPolicy or a Kyverno webhook.
--
-- Documentation:
--   Policy types overview: https://kyverno.io/docs/policy-types/mutating-policy/
--
-- Condition types and reasons are defined in:
--   https://github.com/kyverno/kyverno/tree/main/config/crds/policies.kyverno.io/policies.kyverno.io_mutatingpolicies.yaml
--
-- MutatingPolicy exposes a conditionStatus with a ready boolean and standard
-- Kubernetes conditions, including:
  ...
  if obj.metadata.generation ~= nil and cs.conditions ~= nil then
    for _, condition in ipairs(cs.conditions) do
      if condition.observedGeneration ~= nil and condition.observedGeneration < obj.metadata.generation then
        ...
        hs.message = "Waiting for MutatingPolicy status to reflect latest generation"
        return hs
      end
    end
  end
...
hs.status = "Progressing"
hs.message = "Waiting for MutatingPolicy status"
return hs
