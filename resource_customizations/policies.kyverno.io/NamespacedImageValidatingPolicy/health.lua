-- NamespacedImageValidatingPolicy is a namespace-scoped resource that defines
-- CEL-based image signature and attestation validation rules applied to container
-- images during admission via a Kyverno webhook, scoped to a specific namespace.
--
-- Documentation:
...
--   https://github.com/kyverno/kyverno/tree/main/config/crds/policies.kyverno.io/policies.kyverno.io_namespacedimagevalidatingpolicies.yaml
--
-- NamespacedImageValidatingPolicy exposes a conditionStatus with a ready boolean
-- and standard Kubernetes conditions, including:
  ...
  if obj.metadata.generation ~= nil and cs.conditions ~= nil then
    for _, condition in ipairs(cs.conditions) do
      if condition.observedGeneration ~= nil and condition.observedGeneration < obj.metadata.generation then
        ...
        hs.message = "Waiting for NamespacedImageValidatingPolicy status to reflect latest generation"
        return hs
      end
    end
  end
...
hs.status = "Progressing"
hs.message = "Waiting for NamespacedImageValidatingPolicy status"
return hs
