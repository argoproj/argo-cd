-- ValidatingPolicy is a cluster-scoped resource that defines CEL-based validation
-- rules applied to Kubernetes resources during admission via a generated
-- ValidatingAdmissionPolicy or a Kyverno webhook.
--
-- Documentation:
--   Policy types overview: https://kyverno.io/docs/policy-types/validating-policy/
--
-- Condition types and reasons are defined in:
--   https://github.com/kyverno/kyverno/tree/main/config/crds/policies.kyverno.io/policies.kyverno.io_validatingpolicies.yaml
--
-- ValidatingPolicy exposes a conditionStatus with a ready boolean and standard
  ...
  if obj.metadata.generation ~= nil and cs.conditions ~= nil then
    for _, condition in ipairs(cs.conditions) do
      if condition.observedGeneration ~= nil and condition.observedGeneration < obj.metadata.generation then
        ...
        hs.message = "Waiting for ValidatingPolicy status to reflect latest generation"
        return hs
      end
    end
  end
...
hs.status = "Progressing"
hs.message = "Waiting for ValidatingPolicy status"
return hs
