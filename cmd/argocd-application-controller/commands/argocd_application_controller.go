...
func NewCommand() *cobra.Command {
	...
	command.Flags().StringVar(&shardingAlgorithm, "dynamic-cluster-distribution-enabled", env.ParseBoolFromEnv(common.EnvEnableDynamicClusterDistribution, false), "Enables dynamic cluster distribution.")
...
func getClusterFilter(kubeClient *kubernetes.Clientset, settingsMgr *settings.SettingsManager, shardingAlgorithm string) sharding.ClusterFilterFunction {
	...
	if dynamicClusterDistributionEnabled && appControllerDeployment != nil && appControllerDeployment.Spec.Replicas != nil {
