...
func NewCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		Run: func(c *cobra.Command, _ []string) {
			...
			appsetOpts := server.ApplicationSetOpts{
				...
				EnableNewSCMProviderFiltering: enableNewSCMProviderFiltering,
	...
	command.Flags().BoolVar(&enableNewSCMProviderFiltering, "appset-enable-new-scm-provider-filtering", env.ParseBoolFromEnv("ARGOCD_APPLICATIONSET_CONTROLLER_ENABLE_NEW_SCM_PROVIDER_FILTERING", false), "Enable corrected filter evaluation in the SCM Provider generator. Must match the applicationset controller setting so that dry-run generation agrees with what the controller produces.")
