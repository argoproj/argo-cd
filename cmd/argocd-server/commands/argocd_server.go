...
func NewCommand() *cobra.Command {
	...
	command.Flags().Float64Var(&otlpSampleRatio, "otlp-sample-ratio", env.ParseFloat64FromEnv("ARGOCD_SERVER_OTLP_SAMPLE_RATIO", 1.0, 0.0, 1.0), "Fraction of traces to sample, from 0.0 (none) to 1.0 (all). Parent-based, so downstream services honor the upstream sampling decision")
