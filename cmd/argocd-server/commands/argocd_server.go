...
func NewCommand() *cobra.Command {
	...
	command.Flags().BoolVar(&insecure, "insecure", env.GetBoolVal("ARGOCD_SERVER_INSECURE", false), "Run server without TLS")
	...
	command.Flags().BoolVar(&disableAuth, "disable-auth", env.GetBoolVal("ARGOCD_SERVER_DISABLE_AUTH", false), "Disable client authentication")
	command.Flags().BoolVar(&enableGZip, "enable-gzip", env.GetBoolVal("ARGOCD_SERVER_ENABLE_GZIP", false), "Enable GZIP compression")
	...
	command.Flags().BoolVar(&repoServerPlaintext, "repo-server-plaintext", env.GetBoolVal("ARGOCD_SERVER_PLAINTEXT", false), "Use a plaintext client (non-TLS) to connect to repository server")
	command.Flags().BoolVar(&repoServerStrictTLS, "repo-server-strict-tls", env.GetBoolVal("ARGOCD_SERVER_STRICT_TLS", false), "Perform strict validation of TLS certificates when connecting to repo server")
