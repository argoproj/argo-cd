...
func getClusterResourceWhitelistItemList(values []string) []v1alpha1.ClusterResourceWhitelistItem {
	var res []v1alpha1.ClusterResourceWhitelistItem
	for _, val := range values {
		if parts := strings.Split(val, "/"); len(parts) == 3 {
			res = append(res, v1alpha1.ClusterResourceWhitelistItem{Group: parts[0], Kind: parts[1], Name: parts[2]})
		} else if parts = strings.Split(val, "/"); len(parts) == 2 {
			res = append(res, v1alpha1.ClusterResourceWhitelistItem{Group: parts[0], Kind: parts[1]})
		} else if len(parts) == 1 {
			res = append(res, v1alpha1.ClusterResourceWhitelistItem{Kind: parts[0]})
		}
	}
	return res
}
