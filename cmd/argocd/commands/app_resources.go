...
func printTreeViewAppResourcesNotOrphaned(nodeMapping map[string]argoappv1.ResourceNode, parentChildMapping map[string][]string, parentNodes map[string]void, orphaned bool, listAll bool) {
	w := os.Stdout
	tbl := uitable.New()
	tbl.Separator = "  "
...
func printTreeViewAppResourcesOrphaned(nodeMapping map[string]argoappv1.ResourceNode, parentChildMapping map[string][]string, parentNodes map[string]void, orphaned bool, listAll bool) {
	w := os.Stdout
	tbl := uitable.New()
	tbl.Separator = "  "
	tbl.AddRow("GROUP", "KIND", "NAMESPACE", "NAME", "ORPHANED")
...
func parentChildDetails(nodes []v1alpha1.ResourceNode) (mapUidToNode map[string]argoappv1.ResourceNode, mapParentToChild map[string][]string, parentNode map[string]void) {
...
func printResources(listAll bool, orphaned bool, appResourceTree *v1alpha1.ApplicationTree, output string) {
	...
	if output == "tree" {
		if !orphaned || listAll {
			...
			printTreeViewAppResourcesNotOrphaned(mapUidToNode, mapParentToChild, parentNode, orphaned, listAll)
		}

		if orphaned || listAll {
			mapUidToNode, mapParentToChild, parentNode := parentChildDetails(appResourceTree.OrphanedNodes)
			printTreeViewAppResourcesOrphaned(mapUidToNode, mapParentToChild, parentNode, orphaned, listAll)
		}
