...
func NewGenClusterConfigCommand(pathOpts *clientcmd.PathOptions) *cobra.Command {
	...
	command.Flags().StringVar(&clusterOpts.ServiceAccount, "service-account", "argocd-manager", fmt.Sprintf("System namespace service account to use for kubernetes resource management. If not set then default %p SA will be used", clusterauth.ArgoCDManagerServiceAccount))
