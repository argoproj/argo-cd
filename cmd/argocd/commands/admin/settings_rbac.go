...
`,
		Run: func(c *cobra.Command, args []string) {
			...
			res := checkPolicy(project, subject, action, resource, subResource, builtinPolicy, userPolicy, projectPolicies, defaultRole, matchMode, strict, isLogRbacEnforced)
			if res {
				if !quiet {
					fmt.Println("Yes")
				}
