...
func NewCommand() *cobra.Command {
	...
	command.PersistentFlags().BoolVar(&clientOpts.PromptsEnabled, "force-prompts-enabled", config.GetBoolFlagWithFallback("force-prompts-enabled", localconfig.GetPromptsEnabled(false)), "Force optional interactive prompts to be enabled or disabled, overriding local configuration. If not specified, the local configuration value will be used, which is false by default.")
