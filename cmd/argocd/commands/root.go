...
func NewCommand() *cobra.Command {
	...
	command.PersistentFlags().IntVar(&clientOpts.HttpRetryMax, "http-retry-max", config.GetIntFlag("http-retry-max", 0), "Maximum number of retries for http connection issues and retryable http status codes when talking to Argo CD server")
