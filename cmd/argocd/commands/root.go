...
func NewCommand() *cobra.Command {
	...
	command.PersistentFlags().StringVar(&clientOpts.ProxyAuthToken, "proxy-auth-token", config.GetFlag("proxy-auth-token", env.StringFromEnv(common.EnvProxyAuthToken, "")), fmt.Sprintf("Proxy Authentication token; set this or the %s environment variable", common.EnvProxyAuthToken))
