...
func NewProjectWindowsDisableManualSyncCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "windows disable-manual-sync")
			defer span.Finish()
...
func NewProjectWindowsEnableManualSyncCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			id, err := strconv.Atoi(args[1])
			errors.CheckError(err)
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "windows enable-manual-sync")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectWindowsAddWindowCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "windows add")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectWindowsDeleteCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			id, err := strconv.Atoi(args[1])
			errors.CheckError(err)
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "windows delete")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectWindowsUpdateCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			id, err := strconv.Atoi(args[1])
			errors.CheckError(err)
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "windows update")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectWindowsListCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "windows list")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
