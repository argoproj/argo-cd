...
func NewProjectRoleAddPolicyCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			roleName := args[1]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role add-policy")
			defer span.Finish()
...
func NewProjectRoleRemovePolicyCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			roleName := args[1]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role remove-policy")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectRoleCreateCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			roleName := args[1]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role create")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectRoleDeleteCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			roleName := args[1]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role delete")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectRoleCreateTokenCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			roleName := args[1]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role create-token")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
			duration, err := timeutil.ParseDuration(expiresIn)
			errors.CheckError(err)
			token, err := projIf.CreateToken(ctx, &projectpkg.ProjectTokenCreateRequest{Project: projName, Role: roleName, ExpiresIn: int64(duration.Seconds())})
			errors.CheckError(err)
...
func NewProjectRoleDeleteTokenCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role delete-token")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectRoleListCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role list")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectRoleGetCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName := args[0]
			roleName := args[1]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role get")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
...
func NewProjectRoleAddGroupCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role add-group")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
			proj, err := projIf.Get(ctx, &projectpkg.ProjectQuery{Name: projName})
...
func NewProjectRoleRemoveGroupCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			projName, roleName, groupName := args[0], args[1], args[2]
			span, ctx := opentracing.StartSpanFromContext(context.Background(), "role remove-group")
			defer span.Finish()
			conn, projIf := argocdclient.NewClientOrDie(clientOpts).NewProjectClientOrDie()
			defer util.Close(conn)
			proj, err := projIf.Get(ctx, &projectpkg.ProjectQuery{Name: projName})
