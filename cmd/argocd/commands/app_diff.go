...
func NewApplicationDiffCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	command.Flags().StringArrayVar(&localIncludes, "local-include", []string{"*.yaml", "*.yml", "*.json", "charts/**/*.yaml", "charts/**/*.yml", "charts/**/*.tpl"}, "Used with --server-side-generate, specify patterns of filenames to send. Patterns without a path separator match on filename only; patterns containing '/' match on the relative path and support '**' to span multiple directories (e.g. \"charts/**\" includes all Helm chart files). WARNING: The default patterns 'charts/**/*.yaml', 'charts/**/*.yml', and 'charts/**/*.tpl' will upload only matching files under the charts/ directory. Use --local-include to override the default patterns if you need to include more files, such as 'charts/**' for all files.")
