...
func NewApplicationDiffCommand(clientOpts *argocdclient.ClientOptions) *cobra.Command {
	...
	command.Flags().StringArrayVar(&localIncludes, "local-include", []string{"*.yaml", "*.yml", "*.json", "charts/**"}, "Used with --server-side-generate, specify patterns of filenames to send. Patterns without a path separator match on filename only; patterns containing '/' match on the relative path and support '**' to span multiple directories (e.g. \"charts/**\" includes all Helm chart files). WARNING: The default pattern 'charts/**' will upload all files under the charts/ directory, which may include sensitive files such as private keys or credentials. Use --local-include to override the default patterns and restrict which files are sent.")
