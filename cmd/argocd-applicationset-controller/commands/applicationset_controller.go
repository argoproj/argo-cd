...
func NewCommand() *cobra.Command {
	...
	command := cobra.Command{
		...
		RunE: func(c *cobra.Command, _ []string) error {
			...
			matrixConfig := generators.NewMatrixConfig(maxMatrixChildren)
			...
			topLevelGenerators := generators.GetGenerators(ctx, mgr.GetClient(), k8sClient, namespace, argoCDService, dynamicClient, scmConfig, clusterInformer, matrixConfig)
	...
	command.Flags().IntVar(&maxMatrixChildren, "max-matrix-children", env.ParseNumFromEnv("ARGOCD_APPLICATIONSET_MAX_MATRIX_CHILDREN", 2, 0, math.MaxInt), "Max number of child generators allowed in a matrix generator")
