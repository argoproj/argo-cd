...
func NewCommand() *cobra.Command {
	...
	command.Flags().DurationVar(&shutdownDrainTimeout, "shutdown-drain-timeout", env.ParseDurationFromEnv("ARGOCD_REPO_SERVER_SHUTDOWN_DRAIN_TIMEOUT", 5*time.Second, 0, math.MaxInt32*time.Second), "How long to wait for in-flight requests to finish on shutdown before terminating the commands they are running; 0 terminates them as soon as the signal arrives. Drain timeout plus one and a half times ARGOCD_EXEC_FATAL_TIMEOUT must fit under the pod's grace period, so requests are cancelled before the kubelet's SIGKILL; the default leaves 10s of margin under a default 30s grace period.")
