...
Orphaned Kubernetes resources are top-level namespaced resources that do not belong to any Argo CD Application. For more information on orphaned resources, see [Orphaned Resources Monitoring](./user-guide/orphaned-resources.md).  

To view orphaned resources in the ArgoCD UI, follow these steps:  

1. Click on **Settings** in the sidebar.  
2. Click on **Projects**.  
3. Select the desired project.  
4. Scroll down to the `RESOURCE MONITORING` section.  
5. Click on **Edit** and then enable the monitoring feature.  
6. The section will expand to show the `Enable application warning conditions?` checkbox. click the checkbox to enable it.  
7. Click **Save**.  
8. Navigate back to **Applications** and select an application under the configured project.  
9. In the `Sync Panel`, under `APP CONDITIONS`, you will see the warning we enabled.  
10. Scroll down the sidebar and click **Show Orphaned** below the `HEALTH STATUS` filters to enable it.  
11. You can now see the orphaned resources in the UI.  
