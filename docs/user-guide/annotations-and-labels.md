...
| [`argocd.argoproj.io/ignore-differences-json-pointers`] (#ignore-differences-json-pointer)      | Resource, Application       | Ignores differences at specified JSON Pointer paths on individual resources |
...
### `argocd.argoproj.io/ignore-differences-json-pointers` {#ignore-differences-json-pointer}

- **Target resource(s):** any managed resource; Application (when managed by an ApplicationSet)
- **Possible values:** a comma-separated list of [RFC6901 JSON Pointers](https://tools.ietf.org/html/rfc6901)

Ignores differences at the specified JSON Pointer paths for an individual resource, without requiring a matching entry in the Application's `spec.ignoreDifferences`. The Application must also have `RespectIgnoreDifferences=true` set in its sync options for this annotation to take effect. Resource-level rules are merged with any application-level ignore rules at diff time.

The annotation can also be placed on Application resources generated by an ApplicationSet, allowing per-Application ignore rules to be expressed directly in the ApplicationSet template rather than in each Application's `spec.ignoreDifferences`.
