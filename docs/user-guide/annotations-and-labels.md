...
| [`argocd.argoproj.io/health-aggregate-overrides`](#health-aggregate-overrides) | any | Overrides how a resource's health status contributes to Application health. |
...
### `argocd.argoproj.io/health-aggregate-overrides` { #health-aggregate-overrides }

- **Target resource(s):** any
- **Possible values:** one or more comma-separated `ResourceStatus=ApplicationStatus` mappings

Controls which health status a resource contributes to its parent Application's aggregated health.
The resource keeps its original health status in the resource tree. For example:

```yaml
metadata:
  annotations:
    argocd.argoproj.io/health-aggregate-overrides: 'Suspended=Healthy'
```

Multiple mappings can be specified with commas, such as
`Suspended=Healthy,Progressing=Degraded`. The mapping is evaluated against the resource's actual
health status. A matching annotation mapping takes precedence over `aggregateAs` returned by a
custom or built-in health check. If there is no matching mapping, `aggregateAs` is used when
present; otherwise, the resource's actual status is used. See
[customizing child resource health](../operator-manual/health.md#customizing-child-resource-health-check-in-applications).
