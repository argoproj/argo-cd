...
> because its extension is `.sealed`. Rename such files so a recongnized extension comes last (e.g. `secret.sealed.yaml`), or use the `--directory-include` option 
> to include the file by name (e.g. `secret.sealed.yaml`).
