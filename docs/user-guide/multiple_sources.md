...
Note that the `$values` variable can only be used at the beginning of the value file path. Also note that its path is always relative to the root of the referenced source.
