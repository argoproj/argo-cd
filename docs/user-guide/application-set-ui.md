...
| `DESIRED APPS` | The Applications that _would_ be generated if the proposed spec were applied. |
...
A small badge labeled with the parent's name appears on the Application
node. Clicking it jumps directly to the parent's details page. For an
app-of-apps child, the badge shows the parent Application's name and links to that Application.
For an Application created from an ApplicationSet, it shows the ApplicationSet.

![ApplicationSet owner badge on an Application](../assets/applicationset/Web-UI/appset-owner-badge.png)

### Show / hide the application's parent node

In the Application tree view preferences, a toggle —
**Show application's parent node** — adds the Application's parent as a
synthetic root node on the resource tree. This is useful when you want a single view that includes
both the parent and the Application it produced.

![Show parent node toggle](../assets/applicationset/Web-UI/show-parent-toggle.png)
