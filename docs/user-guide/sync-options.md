...
In situations where individual resources need to have differences ignored independently of the application-level `spec.ignoreDifferences` configuration, the `argocd.argoproj.io/ignore-differences-json-pointers` annotation can be applied directly to the resource manifest:
