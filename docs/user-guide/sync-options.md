...
In situations where individual resources need to have differences ignored independently of the application-level `spec.ignoreDifferences` configuration, the `argocd.argoproj.io/ignore-differences` annotation can be applied directly to the resource manifest:
...
The annotation accepts yaml with key jsonPointers of paths whose diff should be ignored; These resource-level rules are merged with any matching entries in `spec.ignoreDifferences` at diff time. This annotation can also be added to applications generated by ApplicationSets when ignore rules need to be applied only to some of the generated applications.
