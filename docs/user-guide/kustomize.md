...
### Private OCI Helm charts

When using `--enable-helm`, Kustomize invokes Helm internally to pull charts referenced in the `helmCharts` field of the kustomization file. If those charts are stored in a private OCI registry, Argo CD will automatically authenticate using the same repository credentials configured for direct Helm OCI sources.

To use private OCI Helm charts with Kustomize, configure the OCI registry credentials as described in the [private repositories documentation](private-repositories.md#helm), then reference the chart in your kustomization file:
...
helmCharts:
  - name: my-chart
    repo: oci://my-registry.example.com/charts/my-chart
    version: 1.0.0
