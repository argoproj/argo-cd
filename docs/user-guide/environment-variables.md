...
| `ARGOCD_CHANGE_PASSWORD_SSO_TOKEN_MAX_AGE` | maximum age of SSO tokens when changing passwords. Useful for IdPs like Azure Entra ID that issue tokens with clock skew. Expects a "Duration" format (default `5m`).                                      |
