...
* `argocd-repo-server` performs helm pull on every Application revision change, which triggers a download of index.yaml; for large index files (~40MB), this may cause high memory usage. To avoid this, you can enable direct pull by configuring it per Helm repository, either via the Argo CD UI or by editing the repository Secret.
