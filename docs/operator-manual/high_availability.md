...
* For repositories with very large ref advertisements, enable `--git-ls-remote-optimized` to resolve `HEAD`, branches,
  ...
  `git -c protocol.version=2 fetch --dry-run --porcelain --no-tags <repository> HEAD` from a temporary bare repository.
  The results are stored together, so concurrent `HEAD` requests share the same two-query cache fill. The dry-run fetch
  uses a bounded set of `HEAD`-related ref prefixes with protocol v2 servers and does not download objects or update
  refs. If either optimized query fails, Argo CD uses the default go-git resolver as a last resort. Unsupported refs,
  such as `refs/pull/`, continue to use the default resolver directly.
