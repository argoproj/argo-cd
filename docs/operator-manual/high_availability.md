...
* `argocd-repo-server` uses the `ARGOCD_GIT_AUTOPACK_LIMIT` environment variable to specify the [limit for automatic Git repacking](https://git-scm.com/docs/git-gc#Documentation/git-gc.txt-gcautoPackLimit), triggering garbage collection to reduce loose objects and optimize repository performance.
