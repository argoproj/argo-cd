...
* `argocd-repo-server` uses the `ARGOCD_GIT_AUTOPACK_LIMIT` environment variable, or `reposerver.git.autopacklimit` key if you are using `argocd-cmd-params-cm`, to specify the [limit for automatic Git repacking](https://git-scm.com/docs/git-gc#Documentation/git-gc.txt-gcautoPackLimit), triggering garbage collection to reduce loose objects and optimize repository performance. By default, the default value defined by git is `50`.
