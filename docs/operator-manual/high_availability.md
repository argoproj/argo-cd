...
* The `argocd-repo-server` is capable of configuring the [limit for automatic Git repacking](https://git-scm.com/docs/git-gc#Documentation/git-gc.txt-gcautoPackLimit), triggering garbage collection, which helps reduce loose objects and optimize repository performance. By default, git sets this value to `50`. Git autopack limit can be set in one of three ways:

  * By using the `--git-autopack-limit` argument on the repo server.
  * By setting the `reposerver.git.autopacklimit` key if you are using `argocd-cmd-params-cm`.
  * By directly setting the `ARGOCD_GIT_AUTOPACK_LIMIT` environment variable on the repo server.
