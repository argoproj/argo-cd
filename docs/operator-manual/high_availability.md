...
* `argocd-repo-server` performs helm pull on every Application revision change, which triggers a download of index.yaml; for large index files (~40MB), this may cause high memory usage (~900MB). To avoid this, you can now enable direct pull by configuring it per Application, by editing the Application CR (`spec.source.helm.directPull`).
