...
| hydrator_status    | Failed                          | Source Hydrator phase for apps with `spec.sourceHydrator` set. Empty when hydrator is not configured on the Application. Possible values: `Hydrating`, `Failed`, `Hydrated`, `Unknown`.         |
