...
| `argocd_appset_abandoned_applications`            |   gauge   | Number of applications owned by the applicationset but no longer generated by it (`abandoned` in the applicationset's resource status). Reported from `status.abandonedCount`, so it is not affected by resource status truncation. It contains labels for the name and namespace of an applicationset. |
| `argocd_kubectl_client_cert_rotation_age_seconds` |   gauge   | Age of kubectl client certificate rotation.                                                                                                                                                |
| `argocd_kubectl_request_duration_seconds`         | histogram | Latency of kubectl requests.                                                                                                                                                               |
| `argocd_kubectl_dns_resolution_duration_seconds`  | histogram | Latency of kubectl resolver.                                                                                                                                                               |
| `argocd_kubectl_request_size_bytes`               | histogram | Size of kubectl requests.                                                                                                                                                                  |
