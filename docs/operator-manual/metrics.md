...
| phase              | Succeeded                       | Phase of a sync operation. Possible values are: Error, Failed, Running, Succeeded, Terminating. On `argocd_app_info` this is the Application's current/last operation phase, sourced from `status.operationState.phase`, and is empty if no operation has run.  |
