...
| operation_phase    | Failed                          | Phase of the most recent operation on the `argocd_app_info` metric, sourced from `status.operationState.phase`. This value persists after the operation completes. Possible values are: `""` (no operation recorded), `Error`, `Failed`, `Running`, `Succeeded`, `Terminating`. |
