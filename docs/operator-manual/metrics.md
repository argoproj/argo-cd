...
| `argocd_app_sync_blocked`                         |   gauge   | Whether automatic syncs of the application are currently blocked by its project sync windows. 1 means an automatic sync attempt right now would be rejected. 0 when no sync windows are configured. Reports 1 when the project or its windows cannot be evaluated. |
