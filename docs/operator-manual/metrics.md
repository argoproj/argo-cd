...
| `argocd_repo_server_active_requests`            |   gauge   | Number of gRPC requests currently in flight in the repo server. Reflects the load the concurrency limiter (`--max-concurrent-grpc-requests`) acts on; suitable for driving HPA scale-out.       |
