...
v2.4 introduced a new `logs` RBAC resource. Before v2.4, `applications, get` implied permission to get logs. Starting 
with v2.5, you _must_ enable `logs, get` to allow users to view logs via the UI or CLI.

!!! note
    If you followed the [v2.3 to 2.4 recommendation](2.3-2.4.md#enable-logs-rbac-enforcement) to set 
    `server.rbac.log.enforce.enable: "true"`, you are done. Feel free to delete that config item from `argocd-cm`, since 
