...
As long as `ssh_rsa` is not supported as single signature algorithm in Gitlab server, OpenSSH_8.9 is not mandatory on server side. Fallback to alternative OpenSSH version is acknowledged like this:
...
than `rsa-ssh`.
   ...
   Follow the directions from the [OpenSSH 8.8 release notes](https://www.openssh.com/txt/release-8.7):
