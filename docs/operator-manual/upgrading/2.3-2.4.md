...
As long as `ssh-rsa` is not supported as single signature algorithm in Gitlab server, OpenSSH_8.9 is not mandatory on server side. Fallback to alternative OpenSSH version is acknowledged like this:
```
