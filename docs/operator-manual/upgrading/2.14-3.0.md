...
### Replaced Go and Sigs Yaml with Goccy Yaml

The library is more maintained and we wanted to standardize between 3 different Yaml libraries used at least directly by Argo CD code.
One notable differences is handling of time.Duration. Sigs Yaml expects int with nanoseconds, while Goccy Yaml expects a duration like 1m.

#### Migration

If any of your configs such as notifications config use nanoseconds for some parameters, change them to the format like 1m.
