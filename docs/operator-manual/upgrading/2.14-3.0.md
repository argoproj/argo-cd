...
Users who have `server.rbac.log.enforce.enable: "true"` in their `argocd-cm` ConfigMap, are unaffected by this change and don't need to perform any additional action. It is recommended though to delete this setting from the `argocd-cm` ConfigMap after the upgrade.

Users who either have `server.rbac.log.enforce.enable` set to `false` or don't have this setting at all in their `argocd-cm` ConfigMap are affected and have to perform the below remediation steps. 
