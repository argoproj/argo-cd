...
Here is an example command, that syncs such an Application, it can be run after you [are logged in ](../../getting_started.md#4-login-using-the-cli) to Argo CD CLI:
```sh
argocd app sync $YOUR_APP_NAME --sync-options-override-style replace --apply-out-of-sync-only=false
```
Running this command requires [downloading](https://argo-cd.readthedocs.io/en/stable/getting_started/#2-download-argo-cd-cli) the latest Argo CD CLI.

It is also possible to sync such an Application using the UI, with `ApplyOutOfSyncOnly` option unchecked. 
