...
2.4 introduced `logs` as a new RBAC resource. In 2.3 and lower, users with `applications, get` access automatically got logs
access. In 2.4, it became possible to enable logs RBAC enforcement with a flag in argocd-cm ConfigMap:

```yaml
server.rbac.log.enforce.enable: "true"
```

In 3.0, you will have to explicitly grant `logs, get` access, as this flag was deprecated.
