...
An example script, that syncs those Applications and waits for 10s after each sync - you can run it after you are logged in with `argocd` CLI:
```sh
kubectl get applications.argoproj.io -A -o json | jq -r '.items[] | select(.spec.syncPolicy.syncOptions[]? == "ApplyOutOfSyncOnly=true") | .metadata.name' | while read -r app; do echo "Syncing $app..."; if argocd app sync "$app"; then echo "✅ Successfully synced: $app"; else echo "❌ Failed to sync: $app"; fi; sleep 10; done
```
