...
The default value of `server.rbac.log.enforce.enable` was `false`.

Starting from 3.0, The default value of `server.rbac.log.enforce.enable` is `true`. Logs RBAC is enforced by default, meaning the `logs` tab on `pod` view will not be visible without granting explicit `logs, get` permissions to the users/groups/roles requiring it.

For 4.0, this configuration switch will be removed and opting out of logs RBAC enforcement would be unsupported.   

#### Detection

Users who already have `server.rbac.log.enforce.enable: "true"` in `argocd-cm` ConfigMap, are unaffected by this change and don't need to perform any additional action. It is recommended though to remove this setting from the ConfigMap after the upgrade.   

Users who don't have `server.rbac.log.enforce.enable` in `argocd-cm` ConfigMap, are affected and have to perform the below remediation steps.  

Users who have `server.rbac.log.enforce.enable: "false"` in `argocd-cm` ConfigMap, are unaffected but are using an unrecommended configuration and are strongly encouraged to perform the below remediation steps.   
It is also recommended to remove this setting from the ConfigMap after the upgrade.   

#### Remediation

Explicitly grant `logs, get` access.   
...
See this [example](../upgrading/2.3-2.4.md#example-1) for more configuration guidelines.      

#### Quick opt-out (not recommended and unsecure)

Though not recommended, you can opt-out from this change by setting `server.rbac.log.enforce.enable: "false"`  
