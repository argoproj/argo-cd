...
Starting from 3.0, logs RBAC is enforced by default, meaning the `logs` tab on `pod` view will not be visible without granting explicit `logs, get` permissions to the users/groups/roles requiring it.

#### Detection

Users who already have `server.rbac.log.enforce.enable: "true"` in `argocd-cm` ConfigMap, are unaffected by this change and don't need to perform any additional action. 
...
Users who have `server.rbac.log.enforce.enable: "false"` in `argocd-cm` ConfigMap, are unaffected but are using a non-recommended configuration and are strongly encouraged to perform the below remediation steps along with removing the `server.rbac.log.enforce.enable` setting from `argocd-cm` ConfigMap.   
