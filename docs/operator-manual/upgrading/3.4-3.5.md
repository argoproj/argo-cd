...
A new `configHash` field has been added to the `Cluster` object to detect configuration changes. 
The hash is computed from `Server`, `Name`, and `Config` fields and is automatically maintained during API-driven updates.

When a cluster secret is edited directly in Kubernetes (declaratively or via `kubectl`), the `configHash` in the secret may become stale. 
This is transient and self-corrects on the next API update, but applications should trigger an explicit cluster update via the API if they require an immediate refresh.
