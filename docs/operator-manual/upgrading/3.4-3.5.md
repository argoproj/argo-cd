...
### ApplicationSet: Bitbucket Cloud SCM provider now supports bearer token authentication

The `SCMProviderGeneratorBitbucket` CRD field has a new optional `bearerToken` field, allowing Bitbucket Cloud workspace tokens to be used in place of app passwords. `bearerToken` and `appPasswordRef` are mutually exclusive; when `bearerToken` is set the `user` field is not required.
...
No action is required for existing ApplicationSets using `appPasswordRef`.
