...
mTLS is opt-in and disabled by default — no action is required for operators who do not configure it. If you choose to enable mTLS by setting `--client-ca-path` on `argocd-repo-server`, all gRPC clients of the repo-server must also be configured with matching client certificates:
