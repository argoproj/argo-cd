...
Argo CD v3.3 upgrades the bundled Kustomize version to 5.8.0. There are no breaking changes in Kustomize 5.8.0 according
to the [release notes](https://github.com/kubernetes-sigs/kustomize/releases/tag/kustomize%2Fv5.8.0).
However, bear in mind that Kustomize 5.8.0 introduces code to replace the `shlex` library used for parsing arguments in the exec plugin.
...
Also Kustomize 5.8.0 fixes a bug that prevented kustomize propagating namespaces to helm charts[release notes](https://github.com/kubernetes-sigs/kustomize/pull/5940)
