...
### Helm repository CA bundles are merged with system root CAs

When a Helm or OCI Helm repository is configured with a custom CA (`tlsClientConfig.caData`),
Argo CD passes `--ca-file` to Helm for chart pulls and registry operations. Helm uses that file
as the sole TLS trust store.

Starting with Argo CD 3.6, Argo CD merges the repository CA with the system root CAs before
passing `--ca-file` to Helm. This keeps chart downloads working when a private registry redirects
to public HTTPS endpoints (for example Nexus pre-signed S3 URLs) without maintaining a manually
merged CA bundle.

**Impact:**

- Operators who relied on the repository CA alone to restrict Helm TLS trust to their private
  CAs will now also trust the system public roots for Helm operations that use a repository-specific
  CA.
- Operators who manually merged public roots into their repository CA bundle can remove the
  redundant public CAs from that bundle; Argo CD adds system trust automatically.

**Mitigation:** Set `ARGOCD_HELM_MERGE_REPOSITORY_CA_WITH_SYSTEM=false` on the `argocd-repo-server`
Deployment to restore the previous behavior (repository CA only). See
[Custom HTTP User-Agent](../user-guide/private-repositories.md#helm-repository-ca-and-system-trust)
