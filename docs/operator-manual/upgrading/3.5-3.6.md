...
The `pkg/utils/kube/KubectlOptionsRunner.Create()` function now ignores its third argument because the underlying
function `k8s.io/kubectl/pkg/cmd/create.CreateOptions.RunCreate()` has changed, it does not accept this parameter since
kubernetes client v0.37.0. This argument will be removed in the next major Argo CD release.
