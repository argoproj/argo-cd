...
`Progressing` during normal promotion and hydration workflows.
...
- `Healthy` is also returned while waiting for hydration to complete, such as when
  dry SHAs are not yet populated or environments are temporarily out of sync on the
  proposed commit.
- `Progressing` is reserved for controller-level waits, such as when the resource has
  no status yet, is not ready, is being deleted, or the spec generation has not been
  observed.
- `Degraded` is unchanged for reconciliation errors and other failure conditions.

Health messages are unchanged; only the reported status differs for the cases above.

**Impact:**

- Applications that include GitOps Promoter resources may show `Healthy` instead of
  `Progressing` after upgrading, even while a promotion or hydration is underway.
