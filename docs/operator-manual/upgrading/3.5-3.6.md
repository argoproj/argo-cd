...
### Default local include paths now include specific Helm files under `charts/`
...
> [!WARNING]
> If your application source contains non-YAML content under `charts/` that you want transmitted, you must override the include set via repeated `--local-include` flags to extend what should be sent, including
   `["*.yaml", "*.yml", "*.json", "charts/**/*.yaml", "charts/**/*.yml", "charts/**/*.tpl"]` 
   if you need them.
...
- Restore pre-3.6 behavior (YAML/JSON only):
...
- Include all files under `charts/` (previous broad behavior):
