...
uses to verify the Kubernetes API server of any cluster whose secret does not set
`tlsClientConfig.caData`. If all your clusters use certificates from the same internal CA, you can
now configure that CA once instead of copying it into every cluster secret.
