...
### SCM Provider generator filter evaluation can be corrected (opt-in)

The SCM Provider generator has never implemented its documented filter semantics. Conditions are
split into repository-level (`repositoryMatch`, `labelMatch`) and branch-level (`branchMatch`,
`pathsExist`, `pathsDoNotExist`) groups, and the two groups are evaluated independently of which
filter each condition came from. A repository therefore has to satisfy *some* repository-level filter
**and** *some* branch-level filter, which turns the documented OR between separate filters into an
AND. A single filter mixing both levels is classified by whichever condition comes last, so the other
half is evaluated against the wrong group.

**New behavior:**

A corrected evaluation checks each filter as a whole and OR's the results, as documented. It is
**opt-in**, because enabling it changes which Applications an existing `ApplicationSet` generates:
...
- Upgrading changes nothing by default. The flag defaults to `false`, which keeps the existing
  evaluation.
- `ApplicationSet` resources using a single filter, or only repository-level conditions, or only
  branch-level conditions, generate the same Applications either way.
- If you enable it, an `ApplicationSet` that combines repository-level and branch-level conditions in
  separate filters will generally select **more** repositories and branches than before, and in some
  combinations a different set. If `spec.syncPolicy.applicationsSync` permits deletion, Applications
  that are no longer selected are pruned. Use `argocd appset generate` to preview the result before
  enabling it.
- Set the same value on `argocd-server` (`--appset-enable-new-scm-provider-filtering`) so that
  `argocd appset generate` and the UI preview agree with the controller. Setting the ConfigMap key
  covers both components in the default manifests.
