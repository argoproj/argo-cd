...
- **On cancellation** (client disconnect, request context cancelled), the command's process group
  gets SIGTERM, escalating to SIGKILL partway through `ARGOCD_EXEC_FATAL_TIMEOUT` (10s when unset).
  This replaces the immediate SIGKILL of the direct child that `os/exec` does by default, so `git`
  gets to clean up its lock files, and grandchildren are reaped along with it.
...
- Budget the pod's `terminationGracePeriodSeconds` above the drain timeout plus
  `ARGOCD_EXEC_FATAL_TIMEOUT`, so the termination sequence completes before the kubelet's SIGKILL.
  With both at their defaults that is 20s, comfortably under the default 30s grace period. If you
  raise either — for example a long drain window to protect slow plugin generations — raise
