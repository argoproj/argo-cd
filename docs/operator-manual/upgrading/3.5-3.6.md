...
### Repo-server runs `git` with `LC_ALL=C`

Repo-server now recovers a repository wedged by a `*.lock` file left behind by an interrupted `git`
process, and it identifies which lock to remove by matching the path out of git's own error message.
git translates those messages from the inherited locale, so every `git` command the repo-server runs
is now given `LC_ALL=C` to keep them in the form Argo CD parses.

**Impact:**

- No action is needed with the shipped repo-server image.
- If you wrap or replace `git` in a custom image such that its messages are localized, the errors
  Argo CD matches must still carry their C-locale text (currently `Unable to create '<path>': File
  exists`), or stale-lock recovery will silently never run.
- git output in repo-server logs is now always in English, regardless of the container's locale.
