...
- **On cancellation** (client disconnect, request context cancelled), every external command gets
  SIGTERM on its process group, escalating to SIGKILL halfway through the grace period —
  `ARGOCD_EXEC_FATAL_TIMEOUT` (10s when unset), or a fixed 10s for config management plugins. This
  replaces the immediate SIGKILL of the direct child that `os/exec` does by default, so `git` gets to
  clean up its lock files, and grandchildren are reaped along with it rather than orphaned — the
  `git` that `kustomize build` runs for remote bases included. A plugin process is still reaped
  immediately so the cmp-server can answer before the client's deadline; only the rest of its process
  group gets the graceful window.
- **On shutdown** (SIGTERM to the repo-server), gRPC drains for up to
  `--shutdown-drain-timeout` / `ARGOCD_REPO_SERVER_SHUTDOWN_DRAIN_TIMEOUT` (default `5s`, settable
  via the `reposerver.shutdown.drain.timeout` key in `argocd-cmd-params-cm`). Requests still
  running when that window elapses have their commands terminated the same way, on the same
  half-a-grace-period timer — 5s at the defaults. Commands are no longer started once shutdown has
  begun; those calls fail immediately instead.
...
- Budget the pod's `terminationGracePeriodSeconds` above the drain timeout plus one and a half times
  `ARGOCD_EXEC_FATAL_TIMEOUT`, so the termination sequence completes before the kubelet's SIGKILL.
  The worst case is the drain window, then half the grace period before the SIGKILL, then one full
  grace period bounding the wait on a command whose output pipes are still held. At the defaults that
  is 20s, inside the default `terminationGracePeriodSeconds` of 30s. Raise the grace period if you
  raise either timeout, for example a long drain window to protect slow plugin generations.
- A command gets half of `ARGOCD_EXEC_FATAL_TIMEOUT` — 5s at the defaults — between the SIGTERM and
  the SIGKILL, on both cancellation and shutdown. The SIGKILL has to land before the wait on the
  command's output pipes gives up, which is why it is half rather than all of it. Raise
  `ARGOCD_EXEC_FATAL_TIMEOUT` if your commands need longer to clean up, and raise
  `terminationGracePeriodSeconds` with it.
- `ARGOCD_EXEC_FATAL_TIMEOUT` now also bounds how long any command's output pipes may stay open
  after it exits, which matters when a command leaves a background process holding them. Lowering it
  shortens that wait too: a command that would have succeeded can instead fail with
  `exec: WaitDelay expired before I/O complete`. Raise it rather than lower it if you see that.
