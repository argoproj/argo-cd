...
### Kubernetes API request timeout no longer terminates long-running requests

`ARGOCD_K8S_SERVER_SIDE_TIMEOUT` now applies to each attempt of a non-long-running Kubernetes API request. Previously, the value was assigned to the REST client's global timeout, which also terminated watch, streaming log, exec, attach, port-forward, and proxy connections when the configured duration elapsed.
