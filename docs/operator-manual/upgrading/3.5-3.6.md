...
process, identifying the lock by matching git's own error message. Because git translates those
messages from the inherited locale, every `git` command the repo-server runs is now given
`LC_ALL=C`. See [Git Configuration](../git_configuration.md#locale).
