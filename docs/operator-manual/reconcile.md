...
To query logs in Datadog for example, use `@application:*name` filter and make sure not to have your logs sampled. Also, Datadog processes structured fields much better if ArgoCD outputs them as JSON (configurable via `controller.log.format: json` in `argocd-cmd-params-cm` for application controller). Make sure to include debug logs (configurable via `controller.log.level: debug` for application controller), since a lot of logs with timings are on debug level. Remember, updating `argocd-cmd-params-cm` requires restarting the application controller to take effect.
