...
However, this behaviour can be changed by setting the `allowDuplicates` field to `true`. This will allow more than one parameter sets of the **base generator** to have the same values for the merge keys.
...
However, on uncommenting the `allowDuplicates` field, it will result in the following parameter sets, thus allowing non-unique values for the merge keys:
```yaml
- env: staging
  server: https://1.2.3.4
  redis: 'false'
