...
```
  ...
  -h, --help                                           help for argocd-repo-server
      ...
      --shutdown-drain-timeout duration                How long to wait for in-flight requests to finish on shutdown before cancelling them; 0 cancels them as soon as the signal arrives. Keep it below the pod's terminationGracePeriodSeconds, so requests are cancelled before the kubelet's SIGKILL. (default 10s)
