...
```
  ...
  -h, --help                                           help for argocd-repo-server
      ...
      --shutdown-drain-timeout duration                How long to wait for in-flight requests to finish on shutdown before terminating the commands they are running; 0 terminates them as soon as the signal arrives. Drain timeout plus ARGOCD_EXEC_FATAL_TIMEOUT must fit under the pod's grace period, so requests are cancelled before the kubelet's SIGKILL. (default 10s)
