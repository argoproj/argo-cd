...
The `verifyTaggedCommit` determines handling of the annotated tag pointed by `targetRevision` by all the modes except for `none`.
When enabled, both the signature of the tag *and* commit are verified.
Otherwise, only tag's signature is.
...
Specifying `HEAD` (tip of the named branch), or `2.0` will successfully validate, regardless of the `verifyTaggedCommit` config.

However, if `targetRevision` is set to the tag `1.0`, it will only be trusted if `verifyTaggedCommit` is set to `false`.
This is because the tag *is* signed, while the underlying commit *is not*.
...
The commit `C` itself, is not problematic if `targetRevision` is set to the tag `1.0`, and `verifyTaggedCommit` is set to `false`.
BUT, the sync will still be rejected because its ancestors are unsigned. 

Implementation note: Under the hood, getting the revision history is performed using [git rev-list](https://git-scm.com/docs/git-rev-list), which can be consulted for more information.
      ...
      policies:
        - repos: ["*"] # For any repository in the project
          gpg:
            ...
            verifyTaggedCommit: false # If the tag is signed, do not verify the commit signature
            keys:
...
* the repository pattern matching can be regexp, shell-glob or a prefix (or a list of each)?
...
Some of these can be prevented by requiring gpg signitures on git push, but not all.
...
#### Comparison with th original `progressive` mode
