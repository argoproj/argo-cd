...
req := httptest.NewRequest(http.MethodGet, "/auth/backchannel-logout", nil)
...
req := httptest.NewRequest(http.MethodPost, "/auth/backchannel-logout", strings.NewReader("logout_token=TOKEN"))
...
rec := httptest.NewRecorder()
h.ServeHTTP(rec, postRequest("other_field=value"))
...
rec := httptest.NewRecorder()
h.ServeHTTP(rec, postRequest("logout_token=TOKEN"))
...
rec := httptest.NewRecorder()
h.ServeHTTP(rec, postRequest("logout_token=BAD"))
assert.Equal(t, http.StatusBadRequest, rec.Code)
...
rec := httptest.NewRecorder()
h.ServeHTTP(rec, postRequest("logout_token=VALID"))
