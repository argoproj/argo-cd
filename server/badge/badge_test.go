...
func testBadgeConfigProvider(applicationNamespaces []string) configbus.Provider {
	return &configbus.StaticProvider{Fields: configbus.StaticFields{
		ApplicationNamespaces: configbus.Ptr(applicationNamespaces),
		StatusBadgeEnabled:    configbus.Ptr(true),
	}}
}

func testBadgeConfigProviderDisabled(applicationNamespaces []string) configbus.Provider {
	return &configbus.StaticProvider{Fields: configbus.StaticFields{
		ApplicationNamespaces: configbus.Ptr(applicationNamespaces),
		StatusBadgeEnabled:    configbus.Ptr(false),
	}}
}
...
func TestHandlerFeatureIsEnabled(t *testing.T) {
	t.Parallel()
	handler := NewHandler(appclientset.NewSimpleClientset(testApp()), "default", testBadgeConfigProvider([]string{}))
...
func TestHandlerFeatureProjectIsEnabled(t *testing.T) {
	...
	for _, tt := range projectTests {
		...
		handler := NewHandler(appclientset.NewSimpleClientset(objects...), tt.namespace, testBadgeConfigProvider([]string{}))
		rr := httptest.NewRecorder()
...
func TestHandlerNamespacesIsEnabled(t *testing.T) {
	...
	t.Run("Application in allowed namespace", func(t *testing.T) {
		t.Parallel()
		handler := NewHandler(appclientset.NewSimpleClientset(testApp2()), "default", testBadgeConfigProvider([]string{"argocd-test"}))
...
func TestHandlerFeatureIsEnabledKeepFullRevisionIsEnabled(t *testing.T) {
	t.Parallel()
	handler := NewHandler(appclientset.NewSimpleClientset(testApp3()), "argocd-test", testBadgeConfigProvider([]string{""}))
...
func TestHandlerRevisionIsEnabledNoOperationState(t *testing.T) {
	...
	handler := NewHandler(appclientset.NewSimpleClientset(app), "default", testBadgeConfigProvider([]string{}))
	req, err := http.NewRequestWithContext(t.Context(), http.MethodGet, "/api/badge?name=test-app&revision=true", http.NoBody)
	require.NoError(t, err)
...
func TestHandlerFeatureIsDisabled(t *testing.T) {
	t.Parallel()
	handler := NewHandler(appclientset.NewSimpleClientset(testApp()), "default", testBadgeConfigProviderDisabled([]string{}))
	req, err := http.NewRequestWithContext(t.Context(), http.MethodGet, "/api/badge?name=test-app", http.NoBody)
	require.NoError(t, err)
