...
func (s *Server) GetManifests(ctx context.Context, q *application.ApplicationManifestQuery) (*repoapiclient.ManifestResponse, error) {
	...
	manifestInfo, err := repoClient.GenerateManifest(ctx, &repoapiclient.ManifestRequest{
		Repo:              repo,
		Revision:          revision,
...
func (s *Server) RevisionMetadata(ctx context.Context, q *application.RevisionMetadataQuery) (*v1alpha1.RevisionMetadata, error) {
	...
	defer util.Close(conn)
	return repoClient.GetRevisionMetadata(ctx, &repoapiclient.RepoServerRevisionMetadataRequest{Repo: repo, Revision: q.GetRevision()})
}
