...
func (s *Server) List(ctx context.Context, q *ApplicationQuery) (*appv1.ApplicationList, error) {
	...
	for _, a := range appList.Items {
		if s.enf.EnforceClaims(ctx.Value(common.ClaimsSubjectKey), common.ClaimsObjectApplications, common.ClaimsActionGet, appRBACName(a)) {
...
func (s *Server) Create(ctx context.Context, q *ApplicationCreateRequest) (*appv1.Application, error) {
	if !s.enf.EnforceClaims(ctx.Value(common.ClaimsSubjectKey), common.ClaimsObjectApplications, common.ClaimsActionCreate, appRBACName(q.Application)) {
	...
	if apierr.IsAlreadyExists(err) {
		...
		if q.Upsert != nil && *q.Upsert {
			if !s.enf.EnforceClaims(ctx.Value(common.ClaimsSubjectKey), common.ClaimsObjectApplications, common.ClaimsActionUpdate, appRBACName(a)) {
				return nil, grpc.ErrPermissionDenied
			}
...
func (s *Server) Update(ctx context.Context, q *ApplicationUpdateRequest) (*appv1.Application, error) {
	if !s.enf.EnforceClaims(ctx.Value(common.ClaimsSubjectKey), common.ClaimsObjectApplications, common.ClaimsActionUpdate, appRBACName(*q.Application)) {
		return nil, grpc.ErrPermissionDenied
	}
...
func (s *Server) Delete(ctx context.Context, q *ApplicationDeleteRequest) (*ApplicationResponse, error) {
	...
	if !s.enf.EnforceClaims(ctx.Value(common.ClaimsSubjectKey), common.ClaimsObjectApplications, common.ClaimsActionDelete, appRBACName(*a)) {
		return nil, grpc.ErrPermissionDenied
	}
...
func (s *Server) Watch(q *ApplicationQuery, ws ApplicationService_WatchServer) error {
	...
	claims := ws.Context().Value(common.ClaimsSubjectKey)
	done := make(chan bool)
	go func() {
		for next := range w.ResultChan() {
			a := *next.Object.(*appv1.Application)
			if q.Name == nil || *q.Name == "" || *q.Name == a.Name {
				if !s.enf.EnforceClaims(claims, common.ClaimsObjectApplications, common.ClaimsActionGet, appRBACName(a)) {
...
func (s *Server) validateApp(ctx context.Context, spec *appv1.ApplicationSpec) error {
	...
	if !s.enf.EnforceClaims(ctx.Value(common.ClaimsSubjectKey), common.ClaimsObjectProjects, common.ClaimsActionGet, proj.Name) {
...
func (s *Server) PodLogs(q *ApplicationPodLogsQuery, ws ApplicationService_PodLogsServer) error {
	...
	if !s.enf.EnforceClaims(ws.Context().Value(common.ClaimsSubjectKey), common.ClaimsObjectApplications, common.ClaimsActionGet, appRBACName(*a)) {
...
func (s *Server) Sync(ctx context.Context, syncReq *ApplicationSyncRequest) (*appv1.Application, error) {
	...
	if !s.enf.EnforceClaims(ctx.Value(common.ClaimsSubjectKey), common.ClaimsObjectApplications, common.ClaimsActionSync, appRBACName(*a)) {
		return nil, grpc.ErrPermissionDenied
	}
