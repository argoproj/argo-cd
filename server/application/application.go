...
//
// Renaming a *managed child* (a child declared in an app-of-apps parent's Git manifest, or
// an app generated by an ApplicationSet) is rejected: the source of truth still names the old
// child, so the managing owner would recreate or prune it. Such children must be renamed in
// the source.
func (s *Server) Rename(ctx context.Context, q *application.ApplicationRenameRequest) (*v1alpha1.Application, error) {
	appName := q.GetName()
	...
	// Renaming a managed child (of an app-of-apps parent or an ApplicationSet) is not supported
	// through this API: the source of truth (the parent's Git manifest or the generator) still
	// names the old child, so the managing owner would recreate it or prune the renamed one on
	// its next reconcile. Such children must be renamed in the source.
	ownerName, _, err := s.getManagingOwner(ctx, a, appNs)
	if err != nil {
		return nil, fmt.Errorf("error detecting managing owner: %w", err)
	}
	if ownerName != "" {
		return nil, status.Errorf(codes.FailedPrecondition,
			"application %q is managed by %q; rename it in the source (the parent's Git manifest or the ApplicationSet generator), not through this API", appName, ownerName)
	}
