	...
	for _, testData := range tests {
		...
		t.Run(testDataCopy.test, func(t *testing.T) {
			...
			if testDataCopy.expectedErrorContains != "" {
				require.ErrorContains(t, err, testDataCopy.expectedErrorContains, "getClaims should have thrown an error and return an error")
			} else {
				require.NoError(t, err)
			}
		})
...
func TestAuthenticate_3rd_party_JWTs(t *testing.T) {
	...
	for _, testData := range tests {
		...
		t.Run(testDataCopy.test, func(t *testing.T) {
			...
			if testDataCopy.expectedErrorContains != "" {
				require.ErrorContains(t, err, testDataCopy.expectedErrorContains, "Authenticate should have thrown an error and blocked the request")
			} else {
				require.NoError(t, err)
			}
		})
	...
	for _, testData := range tests {
		...
		t.Run(testDataCopy.test, func(t *testing.T) {
			...
			if testDataCopy.expectedErrorMessage != "" {
				require.ErrorContains(t, err, testDataCopy.expectedErrorMessage, "Authenticate should have thrown an error and blocked the request")
			} else {
				require.NoError(t, err)
			}
		})
