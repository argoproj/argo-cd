...
func (s *Server) enforceClusterAccessErr(ctx context.Context, action string, clust *appv1.Cluster) error {
	errWithServer := s.enf.EnforceErr(ctx.Value("claims"), rbac.ResourceClusters, action, CreateClusterRBACObject(clust.Project, clust.Server))
	if errWithServer == nil {
		return nil
	}
	...
	return errors.Join(errWithServer, errWithName)
}
