...
func (s *Server) List(ctx context.Context, q *ClusterQuery) (*appv1.ClusterList, error) {
	...
	if clusterList != nil {
		...
		for _, clust := range clusterList.Items {
			if s.enf.EnforceClaims(ctx.Value(rbac.ClaimsSubjectKey), rbac.ClaimsObjectClusters, rbac.ClaimsActionGet, clust.Server) {
...
func (s *Server) Create(ctx context.Context, q *ClusterCreateRequest) (*appv1.Cluster, error) {
	if !s.enf.EnforceClaims(ctx.Value(rbac.ClaimsSubjectKey), rbac.ClaimsObjectClusters, rbac.ClaimsActionCreate, q.Cluster.Server) {
...
func (s *Server) Get(ctx context.Context, q *ClusterQuery) (*appv1.Cluster, error) {
	if !s.enf.EnforceClaims(ctx.Value(rbac.ClaimsSubjectKey), rbac.ClaimsObjectClusters, rbac.ClaimsActionGet, q.Server) {
		return nil, grpc.ErrPermissionDenied
	}
...
func (s *Server) Update(ctx context.Context, q *ClusterUpdateRequest) (*appv1.Cluster, error) {
	if !s.enf.EnforceClaims(ctx.Value(rbac.ClaimsSubjectKey), rbac.ClaimsObjectClusters, rbac.ClaimsActionUpdate, q.Cluster.Server) {
		return nil, grpc.ErrPermissionDenied
	}
	err := kube.TestConfig(q.Cluster.RESTConfig())
	if err != nil {
...
func (s *Server) Delete(ctx context.Context, q *ClusterQuery) (*ClusterResponse, error) {
	if !s.enf.EnforceClaims(ctx.Value(rbac.ClaimsSubjectKey), rbac.ClaimsObjectClusters, rbac.ClaimsActionDelete, q.Server) {
		return nil, grpc.ErrPermissionDenied
	}
