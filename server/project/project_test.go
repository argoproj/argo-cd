...
func TestProjectServer(t *testing.T) {
	...
	t.Run("TestCreateTokenDenied", func(t *testing.T) {
		...
		require.EqualError(t, err, "rpc error: code = PermissionDenied desc = permission denied: projects, update, test")
	})
	...
	t.Run("TestCreateTokenWithSameIdDeny", func(t *testing.T) {
		...
		require.EqualError(t, err1, expectedErr)
	})
...
p, role:admin, projects, update, *, allow`)
	...
	t.Run("TestGetSyncWindowsStateDenied", func(t *testing.T) {
		...
		require.EqualError(t, err, "rpc error: code = PermissionDenied desc = permission denied: projects, get, test")
	})
}
