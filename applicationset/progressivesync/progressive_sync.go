...
func (m *Manager) PerformProgressiveSyncs(ctx context.Context, logCtx *log.Entry, appset argov1alpha1.ApplicationSet, applications []argov1alpha1.Application, desiredApplications []argov1alpha1.Application, refreshGracePeriodSeconds int) (map[string]bool, error) {
	...
	allReconciled, err := m.ensureApplicationsReconciled(logCtx, &appset, applications, previousWaitingTime, updatedAppsetApplicationStatus, refreshGracePeriodSeconds, appStepMap)
	if err != nil {
...
func (m *Manager) addRefreshAnnotationToApplications(logCtx *log.Entry, appset *argov1alpha1.ApplicationSet, applications []argov1alpha1.Application, appStepMap map[string]int) error {
	for _, app := range applications {
...
func needsReconcile(logCtx *log.Entry, app argov1alpha1.Application, statusMap map[string]argov1alpha1.ApplicationSetApplicationStatus, sinceTime *metav1.Time) bool {
	...
	if appStatus, ok := statusMap[app.Name]; ok {
...
func (m *Manager) ensureApplicationsReconciled(logCtx *log.Entry, appset *argov1alpha1.ApplicationSet, applications []argov1alpha1.Application, previousWaitingTime *metav1.Time, appSetAppStatus []argov1alpha1.ApplicationSetApplicationStatus, refreshGracePeriodSeconds int, appStepMap map[string]int) (bool, error) {
	...
	err := m.addRefreshAnnotationToApplications(logCtx, appset, appsNeedReconcile, appStepMap)
	if err != nil {
