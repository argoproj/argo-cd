...
func (h *WebhookHandler) shouldRefreshMatrixGenerator(gen *v1alpha1.MatrixGenerator, appSet *v1alpha1.ApplicationSet, gitGenInfo *gitGeneratorInfo, prGenInfo *prGeneratorInfo) bool {
	...
	if len(params) != 0 {
		for _, p := range params {
			generators1 := generators.GetRelevantGenerators(requestedGenerator1, h.generators)
			tempInterpolatedGenerator, err := generators.InterpolateGenerator(generators1, requestedGenerator1, p, appSet.Spec.GoTemplate, appSet.Spec.GoTemplateOptions)
