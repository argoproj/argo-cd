...
// fakeETagHash replicates GitHub's ETag hashing strategy used in the fake test
// server: for each header name in varyHeaders (in order), write every value of
// that header from requestHeaders followed by ':', then write the response body.
// The caller is expected to call h.Write(payload) after this returns.
func fakeETagHash(requestHeaders http.Header, varyHeaders []string) hash.Hash {
	h := sha256.New()
	for _, name := range varyHeaders {
		for _, val := range requestHeaders.Values(name) {
			h.Write(append([]byte(val), ':'))
		}
	}
	return h
}
...
func setupFakeGithubServer(cacheHitCounter *int) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		h := fakeETagHash(r.Header, varyHeadersTest)
		h.Write(payload)
...
func TestGitHubCache_Success(t *testing.T) {
	...
	client := &http.Client{
		Transport: NewGitHubCacheTransport(nil, cacheCtx, 100),
	}
	ctx := t.Context()
...
func TestNewGitHubCache(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			client := NewGitHubCacheFrom(&http.Client{Transport: tc.parent}, cacheCtx, 100)
...
func TestGithubHashToken(t *testing.T) {
	tests := []struct {
		name          string
		authorization string
		wantEmpty     bool // true when the return value must be ""
		wantSameAs    string
		...
		{
			name:          "unrecognised scheme returns empty string",
			authorization: "Digest realm=example",
			wantEmpty:     true,
		},
		{
			name:          "Bearer scheme extracts token",
			authorization: "Bearer mytoken",
			wantEmpty:     false,
			wantSameAs:    "Bearer mytoken",
			wantDifferent: "Bearer othertoken",
		},
		{
			name:          "token scheme extracts token",
			authorization: "token mytoken",
			wantEmpty:     false,
			wantSameAs:    "token mytoken",
			wantDifferent: "token othertoken",
		},
		{
			name:          "Basic scheme hashes only the password",
			authorization: "Basic " + base64.StdEncoding.EncodeToString([]byte("user:secret")),
			wantEmpty:     false,
			wantSameAs:    "Basic " + base64.StdEncoding.EncodeToString([]byte("user:secret")),
			wantDifferent: "Basic " + base64.StdEncoding.EncodeToString([]byte("user:othersecret")),
		},
		{
			name: "Basic with same password but different username gives same hash",
			// Only the password is hashed, so a username change must not affect the result.
			authorization: "Basic " + base64.StdEncoding.EncodeToString([]byte("alice:secret")),
			wantSameAs:    "Basic " + base64.StdEncoding.EncodeToString([]byte("bob:secret")),
		},
		{
			// 'Bearer ' and 'token ' are separate scheme prefixes handled by
			// independent else-if branches. A credential that matches the first
			// branch must not also be processed by the second branch (i.e. the
			// extracted token value must be the same regardless of scheme when
			// the raw credential portion is identical).
			name:          "Bearer scheme is not matched by token branch",
			authorization: "Bearer mytoken",
			wantSameAs:    "Bearer mytoken",
			// A different raw credential must produce a different hash even
			// under the same scheme.
			wantDifferent: "Bearer anothertoken",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := githubHashToken(tt.authorization)

			if tt.wantEmpty {
				assert.Empty(t, got, "expected empty hash for unrecognised/empty header")
				return
			}
			...
			if tt.wantSameAs != "" {
				assert.Equal(t, got, githubHashToken(tt.wantSameAs),
					"same credential must produce the same hash")
			}
			if tt.wantDifferent != "" {
				assert.NotEqual(t, got, githubHashToken(tt.wantDifferent),
					"different credential must produce a different hash")
			}
		})
	}
}
...
func TestIsSameVaryHeader(t *testing.T) {
	...
	hashedToken := githubHashToken(tokenOK)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expected, isSameVaryHeaders(req, tt.varyHeaders, tt.varyHeadersValue))
		})
	}
}
...
func TestGitHubCache_NoEtagResponseNotCached(t *testing.T) {
	...
	client := &http.Client{Transport: NewGitHubCacheTransport(nil, cacheCtx, 100)}
	ctx := t.Context()
