...
var (
	...
	descAppsetAbandonedApps = prometheus.NewDesc(
		"argocd_appset_abandoned_applications",
		"Number of applications owned by the applicationset but no longer generated by it",
		descAppsetDefaultLabels,
		nil,
	)
)
...
func collectAppset(appset *argoappv1.ApplicationSet, labelsToCollect []string, ch chan<- prometheus.Metric) {
	...
	// Read the dedicated count rather than counting status.resources entries: the resource status
	// list may be truncated to --max-resources-status-count, the count is not.
	ch <- prometheus.MustNewConstMetric(descAppsetAbandonedApps, prometheus.GaugeValue, float64(appset.Status.AbandonedCount), appset.Namespace, appset.Name)
}
