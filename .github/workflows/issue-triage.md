...
**Unsupported Argo CD versions:** Extract the version from the issue's `argocd version` field only. Use `web-fetch` on https://github.com/argoproj/argo-cd/releases to determine the 3 most recent minor versions (Argo CD supports the last 3 minor versions per its security policy). If the version in the `argocd version` field is older than those 3, add a polite comment stating it is EOL, listing the supported versions, and asking whether the issue is reproducible on a supported version.
