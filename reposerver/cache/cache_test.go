...
func TestCache_GetManifests(t *testing.T) {
	...
	err := cache.GetManifests("my-revision", &ApplicationSource{}, q, "my-namespace", "", "my-app-label-key", "my-app-label-value", value, "test")
	assert.Equal(t, ErrCacheMiss, err)
	// populate cache
	res := &CachedManifestResponse{ManifestResponse: &apiclient.ManifestResponse{SourceType: "my-source-type"}}
	err = cache.SetManifests("my-revision", &ApplicationSource{}, q, "my-namespace", "", "my-app-label-key", "my-app-label-value", res, "test")
	assert.NoError(t, err)
	// cache miss
	err = cache.GetManifests("other-revision", &ApplicationSource{}, q, "my-namespace", "", "my-app-label-key", "my-app-label-value", value, "test")
	assert.Equal(t, ErrCacheMiss, err)
	// cache miss
	err = cache.GetManifests("my-revision", &ApplicationSource{Path: "other-path"}, q, "my-namespace", "", "my-app-label-key", "my-app-label-value", value, "test")
	assert.Equal(t, ErrCacheMiss, err)
	// cache miss
	err = cache.GetManifests("my-revision", &ApplicationSource{}, q, "other-namespace", "", "my-app-label-key", "my-app-label-value", value, "test")
	assert.Equal(t, ErrCacheMiss, err)
	// cache miss
	err = cache.GetManifests("my-revision", &ApplicationSource{}, q, "my-namespace", "", "other-app-label-key", "my-app-label-value", value, "test")
	assert.Equal(t, ErrCacheMiss, err)
	// cache miss
	err = cache.GetManifests("my-revision", &ApplicationSource{}, q, "my-namespace", "", "my-app-label-key", "other-app-label-value", value, "test")
	assert.Equal(t, ErrCacheMiss, err)
	// cache hit
	err = cache.GetManifests("my-revision", &ApplicationSource{}, q, "my-namespace", "", "my-app-label-key", "my-app-label-value", value, "test")
	assert.NoError(t, err)
...
func TestCachedManifestResponse_HashBehavior(t *testing.T) {
	...
	err := repoCache.SetManifests(response.Revision, appSrc, &apiclient.ManifestRequest{}, response.Namespace, "", appKey, appValue, store, "test")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = repoCache.GetManifests(response.Revision, appSrc, &apiclient.ManifestRequest{}, response.Namespace, "", appKey, appValue, retrievedVal, "test")
	if err != nil {
		t.Fatal(err)
	}
