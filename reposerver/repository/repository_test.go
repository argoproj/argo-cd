...
func TestGenerateYamlManifestInDir(t *testing.T) {
	...
	res2, err := GenerateManifests("./testdata/concatenated", "/", "", &q, false, 0)
	assert.NoError(t, err)
...
func TestIdentifyAppSourceTypeByAppDirWithKustomizations(t *testing.T) {
	sourceType, err := GetAppSourceType(&argoappv1.ApplicationSource{}, "./testdata/kustomization_yaml", "testapp", 0)
	assert.Nil(t, err)
	assert.Equal(t, argoappv1.ApplicationSourceTypeKustomize, sourceType)

	sourceType, err = GetAppSourceType(&argoappv1.ApplicationSource{}, "./testdata/kustomization_yml", "testapp", 0)
	assert.Nil(t, err)
	assert.Equal(t, argoappv1.ApplicationSourceTypeKustomize, sourceType)

	sourceType, err = GetAppSourceType(&argoappv1.ApplicationSource{}, "./testdata/Kustomization", "testapp", 0)
	assert.Nil(t, err)
	assert.Equal(t, argoappv1.ApplicationSourceTypeKustomize, sourceType)
}
...
func TestGenerateFromUTF16(t *testing.T) {
	...
	res1, err := GenerateManifests("./testdata/utf-16", "/", "", &q, false, 0)
	assert.Nil(t, err)
	assert.Equal(t, 2, len(res1.Manifests))
}
