...
func newCacheMocks() *repoCacheMocks {
	...
	return &repoCacheMocks{
		...
		cache:          cache.NewCache(cacheutilCache, 1*time.Minute, 1*time.Minute, 1*time.Minute),
...
func TestGenerateManifests_K8SAPIResetCache(t *testing.T) {
	...
	cachedFakeResponse := &apiclient.ManifestResponse{Manifests: []*apiclient.Manifest{{CompiledManifest: "Fake"}}}
