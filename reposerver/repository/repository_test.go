...
func TestFlattenValues(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "array of maps",
			input:    map[string]any{"hosts": []any{map[string]any{"host": "chart.local", "path": "/"}}},
			expected: map[string]string{"hosts[0].host": "chart.local", "hosts[0].path": "/"},
		},
		{
			name:     "empty map",
			input:    map[string]any{},
...
func Test_populateHelmAppDetails_WithInlineValues(t *testing.T) {
	...
	appPath, err := filepath.Abs("./testdata/values-files/")
	require.NoError(t, err)
	err = service.populateHelmAppDetails(t.Context(), &res, appPath, appPath, "dummy_sha", "main", &q, emptyTempPaths)
	require.NoError(t, err)
