...
func TestListApps(t *testing.T) {
	...
	expectedApps := map[string]string{
		...
		"helm-with-dependencies-using-alias":"Helm",
	}
...
func TestGetHelmRepo_NamedReposUsingAlias(t *testing.T) {
	src := argoappv1.ApplicationSource{Path: "."}
	q := apiclient.ManifestRequest{Repo: &argoappv1.Repository{}, ApplicationSource: &src, Repos: []*argoappv1.Repository{{
		Name:     "custom-repo-using-alias",
		Repo:     "https://example.com",
		Username: "test",
	}}}

	helmRepos, err := getHelmRepos("./testdata/helm-with-dependencies-using-alias", q.Repos, q.HelmRepoCreds)
	assert.Nil(t, err)
