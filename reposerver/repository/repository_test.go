...
func TestRunCustomTool(t *testing.T) {
	res, err := GenerateManifests(".", ".", &repoapiclient.ManifestRequest{
...
func TestGetAppDetailsHelm(t *testing.T) {
	...
	{
		res, err := serve.GetAppDetails(ctx, &repoapiclient.RepoServerAppDetailsQuery{
	...
	// verify values specific parameters are returned when a values is specified
	{
		res, err := serve.GetAppDetails(ctx, &repoapiclient.RepoServerAppDetailsQuery{
			Repo: &argoappv1.Repository{Repo: "https://github.com/fakeorg/fakerepo.git"},
			Path: "redis",
			Helm: &repoapiclient.HelmAppDetailsQuery{
...
func TestService_GetRevisionMetadata(t *testing.T) {
	...
	type args struct {
		q *repoapiclient.RepoServerRevisionMetadataRequest
	}
	q := &repoapiclient.RepoServerRevisionMetadataRequest{Repo: &argoappv1.Repository{}, Revision: factory.revision}
