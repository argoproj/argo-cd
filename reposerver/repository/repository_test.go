...
func newCacheMocksWithOpts(repoCacheExpiration, revisionCacheExpiration, revisionCacheLockTimeout time.Duration) *repoCacheMocks {
	...
	return &repoCacheMocks{
		...
		cache:          cache.NewCache(cacheutilCache, repoCacheExpiration, revisionCacheExpiration, revisionCacheLockTimeout, 1*time.Minute),
...
func TestGenerateYamlManifestInDir(t *testing.T) {
	...
	const countOfManifests = 48 // khulnasoft has only 48 because notification not included but we have event-reporter (count yamls in /manifests/base)
...
func TestGenerateManifests_K8SAPIResetCache(t *testing.T) {
	...
	cachedFakeResponse := &apiclient.ManifestResponse{Manifests: []*apiclient.Manifest{{CompiledManifest: "Fake"}}, Revision: mock.Anything}
...
func TestNilMetadataAccessors(t *testing.T) {
	...
	assert.Equal(t, expected, res.Manifests[0].CompiledManifest)
}
...
func TestFindResources(t *testing.T) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var names []string
			for i := range objs {
				names = append(names, objs[i].obj.GetName())
			}
...
func TestFindManifests_Exclude(t *testing.T) {
	...
	assert.Equal(t, "nginx-deployment", objs[0].obj.GetName())
}
