...
func (s *Service) runManifestGen(ctx context.Context, repoRoot, commitSHA, cacheKey string, opContextSrc operationContextSrc, q *apiclient.ManifestRequest) (*apiclient.ManifestResponse, error) {
	...
	if err == nil {
		manifestGenResult, err = GenerateManifests(ctx, opContext.appPath, repoRoot, commitSHA, q, false, s.gitCredsStore, gitClient)
	}
	if err != nil {
...
func GenerateManifests(ctx context.Context, appPath, repoRoot, revision string, q *apiclient.ManifestRequest, isLocal bool, gitCredsStore git.CredsStore, gitClient git.Client) (*apiclient.ManifestResponse, error) {
	var (
		manifests []manifest
	...
	case v1alpha1.ApplicationSourceTypeKsonnet:
		manifests, dest, err = ksShow(q.AppLabelKey, appPath, repoRoot, q.ApplicationSource.Ksonnet)
	...
	case v1alpha1.ApplicationSourceTypePlugin:
		if q.ApplicationSource.Plugin != nil && q.ApplicationSource.Plugin.Name != "" {
			...
			manifests, err = runConfigManagementPluginSidecars(ctx, appPath, repoRoot, env, q, q.Repo.GetGitCreds(gitCredsStore))
			if err != nil {
...
func ksShow(appLabelKey, appPath, repoRoot string, ksonnetOpts *v1alpha1.ApplicationSourceKsonnet) ([]manifest, *v1alpha1.ApplicationDestination, error) {
	...
	manifests := make([]manifest, len(targetObjs))
	...
	err = filepath.Walk(appPath, func(path string, f os.FileInfo, err error) error {
		...
		if strings.HasSuffix(f.Name(), ".jsonnet") {
			...
			if strings.HasSuffix(f.Name(), ".json") {
				...
				objs, err := expandUnstructuredObjs(yamlObjs)
				if err != nil {
					return err
				}
...
func runConfigManagementPlugin(appPath, repoRoot string, envVars *v1alpha1.Env, q *apiclient.ManifestRequest, creds git.Creds) ([]manifest, error) {
...
func runConfigManagementPluginSidecars(ctx context.Context, appPath, repoPath string, envVars *v1alpha1.Env, q *apiclient.ManifestRequest, creds git.Creds) ([]manifest, error) {
