...
func (s *Service) ListDir(ctx context.Context, q *repoapiclient.ListDirRequest) (*repoapiclient.FileList, error) {
	...
	if files, err := s.cache.GetGitListDir(commitSHA, q.Path); err == nil {
		...
		return &repoapiclient.FileList{Items: files}, nil
	}
	...
	res := repoapiclient.FileList{Items: lsFiles}
...
func (s *Service) GetFile(ctx context.Context, q *repoapiclient.GetFileRequest) (*repoapiclient.GetFileResponse, error) {
	gitClient, commitSHA, err := s.newClientResolveRevision(q.Repo, q.Revision)
	if err != nil {
		return nil, err
	}
	...
	if data, err := s.cache.GetGitFile(commitSHA, q.Path); err == nil {
		...
		return &repoapiclient.GetFileResponse{Data: data}, nil
	}
	...
	res := repoapiclient.GetFileResponse{
		Data: data,
	}
...
func (s *Service) GenerateManifest(c context.Context, q *repoapiclient.ManifestRequest) (*repoapiclient.ManifestResponse, error) {
	gitClient, commitSHA, err := s.newClientResolveRevision(q.Repo, q.Revision)
	if err != nil {
		return nil, err
	}
	getCached := func() *repoapiclient.ManifestResponse {
...
func GenerateManifests(root, path string, q *repoapiclient.ManifestRequest) (*repoapiclient.ManifestResponse, error) {
...
func runConfigManagementPlugin(appPath string, q *repoapiclient.ManifestRequest, creds git.Creds) ([]*unstructured.Unstructured, error) {
...
func (s *Service) GetAppDetails(ctx context.Context, q *repoapiclient.RepoServerAppDetailsQuery) (*repoapiclient.RepoAppDetailsResponse, error) {
	revision := q.Revision
	if revision == "" {
		revision = "HEAD"
	}
	...
	getCached := func() *repoapiclient.RepoAppDetailsResponse {
		var res repoapiclient.RepoAppDetailsResponse
	...
	res := repoapiclient.RepoAppDetailsResponse{
		Type: string(appSourceType),
	}

	switch appSourceType {
	case v1alpha1.ApplicationSourceTypeKsonnet:
		var ksonnetAppSpec repoapiclient.KsonnetAppSpec
	...
	case v1alpha1.ApplicationSourceTypeHelm:
		res.Helm = &repoapiclient.HelmAppSpec{}
	...
	case v1alpha1.ApplicationSourceTypeKustomize:
		res.Kustomize = &repoapiclient.KustomizeAppSpec{}
...
func (s *Service) GetRevisionMetadata(ctx context.Context, q *repoapiclient.RepoServerRevisionMetadataRequest) (*v1alpha1.RevisionMetadata, error) {
...
func valueFiles(q *repoapiclient.RepoServerAppDetailsQuery) []string {
	if q.Helm == nil {
		return nil
	}
