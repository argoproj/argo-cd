...
func GenerateManifests(appPath, repoRoot, revision string, q *apiclient.ManifestRequest, isLocal bool) (*apiclient.ManifestResponse, error) {
	...
	case v1alpha1.ApplicationSourceTypeKustomize:
		manifests, err = kustomizeBuild(repoURL, repoRoot, appPath, q.Repo.GetGitCreds(), q.ApplicationSource.Kustomize, q.KustomizeOptions)
	case v1alpha1.ApplicationSourceTypePlugin:
		manifests, err = runConfigManagementPlugin(appPath, env, q, q.Repo.GetGitCreds())
	case v1alpha1.ApplicationSourceTypeDirectory:
		var directory *v1alpha1.ApplicationSourceDirectory
		if directory = q.ApplicationSource.Directory; directory == nil {
			directory = &v1alpha1.ApplicationSourceDirectory{}
		}
		manifests, err = findManifests(appPath, repoRoot, env, *directory)
	}
	if err != nil {
		return nil, err
	}
	...
	for i, m := range manifests {
		if q.AppLabelKey != "" && q.AppName != "" && !kube.IsCRD(m.obj) {
			err = argokube.SetAppInstanceLabel(m.obj, q.AppLabelKey, q.AppName)
			if err != nil {
				return nil, err
			}
		}
...
func kustomizeBuild(
	...
	relPath, err := filepath.Rel(repoRoot, appPath)
	if err != nil {
		return nil, err
	}
...
func findManifests(appPath string, repoRoot string, env *v1alpha1.Env, directory v1alpha1.ApplicationSourceDirectory) ([]manifest, error) {
	var manifests []manifest
...
func runConfigManagementPlugin(appPath string, envVars *v1alpha1.Env, q *apiclient.ManifestRequest, creds git.Creds) ([]manifest, error) {
	concurrencyAllowed := isConcurrencyAllowed(appPath)
	if !concurrencyAllowed {
		manifestGenerateLock.Lock(appPath)
		defer manifestGenerateLock.Unlock(appPath)
	...
	objects, err := kube.SplitYAML([]byte(out))
	if err != nil {
		return nil, fmt.Errorf("failed to split objects: %w", err)
	}
