...
func TestAcquireParallelismSlot_NoSemaphore(t *testing.T) {
	...
	release, err := acquireParallelismSlot(t.Context(), operationSettings{}, nil)
...
func TestAcquireParallelismSlot_Blocking(t *testing.T) {
	...
	release, err := acquireParallelismSlot(t.Context(), settings, nil)
	require.NoError(t, err)
	...
	cancel()
	_, err = acquireParallelismSlot(ctx, settings, nil)
	require.Error(t, err)
	...
	release()
	release2, err := acquireParallelismSlot(t.Context(), settings, nil)
	require.NoError(t, err)
	release2()
}
...
func TestAcquireParallelismSlot_FailFast(t *testing.T) {
	...
	// Fill all slots.
	release1, err := acquireParallelismSlot(t.Context(), settings, nil)
	require.NoError(t, err)
	release2, err := acquireParallelismSlot(t.Context(), settings, nil)
	require.NoError(t, err)
	...
	start := time.Now()
	_, err = acquireParallelismSlot(t.Context(), settings, nil)
	require.Error(t, err)
	...
	release1()
	release3, err := acquireParallelismSlot(t.Context(), settings, nil)
	require.NoError(t, err)
