    ...
    interface Window {
        extensionsAPI: {
            registerStatusPanelExtension: (
                ...
                title: string,
                key: string,
                flyout?: React.ComponentType<any>
            ) => void;
        };
    }
}
...
const sectionLabel = (title: string, helpContent?: string) => (
    React.createElement(
        ...
        React.createElement(
            'label',
            {style: {fontSize: '12px', fontWeight: 600, color: ARGO_GRAY6_COLOR}},
            [
                title,
...
const ProgressiveSyncPanel = (props: {
    ...
    return React.createElement(
        ...
        [
            sectionLabel(
                'PROGRESSIVE SYNC',
                'Shows if the application is currently in a rolling sync state, waiting for other applications to sync.'
            ),
            React.createElement(
                'div',
...
const ProgressiveSyncFlyout = (props: {
    application: Application;
    ...
    return React.createElement(
        ...
        [
            ...
            React.createElement('div', { style: { marginBottom: '1em' } }, [
                ...
                React.createElement('div', null, [
                    ...
                    React.createElement('ul', null,
                        syncOptions.map((option, i) =>
                            React.createElement('li', { key: i }, option)
                        )
                    )
                ])
            ]),
            React.createElement('div', null, [
                ...
                React.createElement('div', null, [
                    React.createElement('strong', null, 'Synced: '),
                    `${syncedResources.length} resources`
                ]),
                React.createElement('div', null, [
            ...
            pendingResources.length > 0 && React.createElement('div', null, [
                React.createElement('h5', null, 'Pending Resources'),
                React.createElement('ul', null,
                    pendingResources.map((resource, i) =>
                        React.createElement('li', { key: i }, `${resource.kind}/${resource.name}`)
                    )
                )
            ])
        ]
...
((window) => {
    if (window.extensionsAPI) {
        window.extensionsAPI.registerStatusPanelExtension(
            ProgressiveSyncPanel,
            'Progressive Sync',
            'progressive_sync',
            ProgressiveSyncFlyout
        );
    }
})(window); 