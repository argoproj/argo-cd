    ...
    return (
        ...
        {(pref) => {
            ...
            function paginator() {
                return (
                    <React.Fragment>
                        {pageCount > 1 && <ReactPaginate containerClassName='paginate__paginator'
                            forcePage={page} pageCount={pageCount} pageRangeDisplayed={5} marginPagesDisplayed={2} onPageChange={(item) => onPageChange(item.selected)} />}
                        <div className='paginate__size-menu'>
                            <DropDownMenu anchor={() => (
                                <a>page size: {pageSize === -1 ? 'all' : pageSize} <i className='fa fa-caret-down'/></a>
