...
export class RepositoriesService {
    ...
    public createHTTPS({type, name, url, username, password, tlsClientCertData, tlsClientCertKey, insecure, enableLfs, fetchRefspecs}:
        {type: string, name: string, url: string, username: string, password: string, tlsClientCertData: string, tlsClientCertKey: string,
            insecure: boolean, enableLfs: boolean, fetchRefspecs: string[]}): Promise<models.Repository> {
        return requests.post('/repositories').send({type, name, repo: url, username, password, tlsClientCertData, tlsClientCertKey, insecure, enableLfs, fetchRefspecs })
            .then((res) => res.body as models.Repository);
    }

    public createSSH({type, name, url, sshPrivateKey, insecure, enableLfs, fetchRefspecs}:
        {type: string, name: string, url: string, sshPrivateKey: string, insecure: boolean, enableLfs: boolean, fetchRefspecs: string[]}): Promise<models.Repository> {
        return requests.post('/repositories').send({ type, name, repo: url, sshPrivateKey, insecure, enableLfs, fetchRefspecs }).then((res) => res.body as models.Repository);
    }
