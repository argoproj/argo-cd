    ...
    <div className='argo-table-list argo-table-list--clickable'>
        <div className='argo-table-list__head'>
            <div className='row'>
                ...
                <div className='columns small-2'>GROUP/KIND</div>
        ...
        {resources.sort((first, second) => nodeKey(first).localeCompare(nodeKey(second))).map((res) => (
            <div key={nodeKey(res)} className='argo-table-list__row' onClick={() => onNodeClick(nodeKey(res))}>
                <div className='row'>
                    ...
                    <div className='columns small-2'>{[res.group, res.kind].filter((item) => !!item).join('/')}</div>
                    <div className='columns small-4'>{res.namespace}</div>
