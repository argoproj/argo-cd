...
import {ApplicationSyncWindowStatusIcon, AppSetHealthStatusIcon, ComparisonStatusIcon, getAppDefaultSource, getAppOperationState, getAppSetConditionCategory} from '../utils';
...
export const ApplicationStatusPanel = ({application, showDiff, showOperation, showConditions, showMetadataInfo}: Props) => {
    ...
    if  ("resources" in application.status) {
        isApplicationSet = false;
    }
    const today = new Date();
    let daysSinceLastSynchronized = 0;
    console.log("============================ isAppSet " + isApplicationSet);
    if (!isApplicationSet) {
        ...
        var appOrig = application as models.Application;
        const cntByCategory = (appOrig.status.conditions || []).reduce(
        ...
        var appSet = application as models.ApplicationSet;
        const cntByCategory = (appSet.status.conditions || []).reduce(
            (map, next) => map.set(getAppSetConditionCategory(next), (map.get(getAppSetConditionCategory(next)) || 0) + 1),
            new Map<string, number>()
        );
        const infos = cntByCategory.get('info');
        const warnings = cntByCategory.get('warning');
        ...
        return (
            <div className='application-status-panel row'>
                <div className='application-status-panel__item'>
                    <div style={{marginBottom: '1em'}}>{sectionLabel({title: 'APP HEALTH', helpContent: 'The health status of your app'})}</div>
                    <div className='application-status-panel__item-value'>
                        <AppSetHealthStatusIcon state={appSet.status} />
                        &nbsp;
                        {appSet.status.conditions ? appSet.status.conditions[0].status : 'Unknown'}
                    </div>
                    {appSet.status.conditions ? (appSet.status.conditions[0].message && <div className='application-status-panel__item-name'>{appSet.status.conditions[0].message}</div>) : (<div></div>)}
                </div>
