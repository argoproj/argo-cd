...
export const ApplicationsSyncPanel = ({show, apps, hide}: {
    ...
    return (
        <Consumer>
            {(ctx) => <SlidingPanel isMiddle={true} isShown={show} onClose={() => hide()} header={(
                <div>
                ...
                <Form onSubmit={async (params: any) => {
                    ...
                    <React.Fragment>
                        <div className='argo-form-row'>
                            ...
                            <label>
                                Apps (
                                <a onClick={() => apps.forEach((app) => formApi.setValue('app/' + app.metadata.name, true))}>all</a>
                                /
                                <a onClick={() => apps.forEach((app) => formApi.setValue('app/' + app.metadata.name, app.status.sync.status === models.SyncStatuses.OutOfSync))}>out
                                    of sync</a>
                                /
