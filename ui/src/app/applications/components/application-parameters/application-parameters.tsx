...
export const ApplicationParameters = (props: {
    ...
    if (props.detailsList && props.detailsList.length > 1) {
        ...
        const repoAppDetails = props.details;
        return getEditablePanel(attributes, repoAppDetails, 0, v);
    }
    ...
    function getEditablePanel(panel: EditablePanelItem[], repoAppDetails: models.RepoAppDetails, ind: number, sources: models.ApplicationSource[]): any {
        ...
        if (sources.length > 1) {
            if (repoAppDetails.type === 'Directory') {
                description = 'REPO URL=' + src.repoURL + '' + (src.path ? ', PATH=' + src.path : '' + (src.targetRevision ? ', TARGET REVISION=' + src.targetRevision : ''));
                ...
                description = 'REPO URL=' + src.repoURL + ', VERSION=' + src.kustomize.version;
            } else if (repoAppDetails.type === 'Plugin') {
                description = 'REPO URL=' + src.repoURL + '' + (src.path ? ', PATH=' + src.path : '' + (src.targetRevision ? ' , ' + src.targetRevision : ''));
            }
        }
        return (
            <EditablePanel
