...
export const ApplicationNodeInfo = (props: {
    ...
    readonly?: boolean;
    // generatedAppNode indicates the node is an Application generated by an ApplicationSet. In this
    // read-only view there is no controlled/managed state, so health and sync status are read directly
    // from the live Application's own status instead of from the parent's status.resources.
    ...
    if (props.live) {
        ...
        } else if (props.node.kind === 'Gateway' && props.node.group === 'gateway.networking.k8s.io') {
            let hostNames = '';
            const status = props.live.status;
    ...
    if (props.generatedAppNode && props.live && props.live.status) {
        // Order STATUS then HEALTH to match the controlled/managed attributes block above.
        const appStatus = props.live.status as models.ApplicationStatus;
        if (appStatus.sync) {
            attributes.push({
                title: 'STATUS',
                value: (
                    <span>
                        <ComparisonStatusIcon status={appStatus.sync.status} label={true} />
                    </span>
                )
            } as any);
        }
        if (appStatus.health) {
            attributes.push({
                ...
                value: (
                    <span>
                        <HealthStatusIcon state={appStatus.health} /> {appStatus.health.status}
                    </span>
                )
            } as any);
            if (appStatus.health.message) {
                attributes.push({title: 'HEALTH DETAILS', value: appStatus.health.message});
            }
        }
    }
    if (props.links) {
