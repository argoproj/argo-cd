                ...
                <div className='application-resources-diff'>
                    ...
                    {files
                        ...
                        .map((file: any) => {
                            ...
                            const isLargeFile = processedStates.some(
                                state => state.name === file.newPath && (state.performanceInfo.infoA?.isLarge || state.performanceInfo.infoB?.isLarge)
                            );

                            if (isLargeFile) {
                                return <VirtualizedDiffSection key={file.newPath} file={file} showPath={showPath} whiteBox={whiteBox} viewType={viewType} />;
                            } else {
                                return <IndividualDiffSection key={file.newPath} file={file} showPath={showPath} whiteBox={whiteBox} viewType={viewType} />;
                            }
                        })}
                </div>
            );
