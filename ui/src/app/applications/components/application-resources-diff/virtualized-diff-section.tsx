...
import {analyzeYamlPerformance} from '../../../shared/utils/yaml-performance';
                ...
                <div>
                    ...
                    {hasMore && (
                        <div className='application-resources-diff__load-more'>
                            <button className='argo-button argo-button--base' onClick={loadMore}>
                                Load More ({Math.max(0, file.hunks.reduce((acc: number, hunk: any) => acc + hunk.changes.length, 0) - visibleChunks * LINES_PER_CHUNK)} lines
                                remaining)
                            </button>
                        </div>
                    )}
