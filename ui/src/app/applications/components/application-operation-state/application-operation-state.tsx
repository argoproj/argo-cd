...
export const ApplicationOperationState: React.StatelessComponent<Props> = ({application, operationState}, ctx: AppContext) => {
    ...
    if (operationState.syncResult) {
        operationAttributes.push({title: 'REVISION', value: <Revision repoUrl={utils.getAppDefaultSource(application).repoURL} revision={utils.getAppDefaultSyncRevision(application)+utils.getAppDefaultSyncRevisionExtra(application)} />});
    }
    let initiator = '';
