...
describe('Help CLI download buttons', () => {
    ...
    test('renders configured download buttons and hides the GitHub releases link when keys are set', async () => {
        mockSettings({'argocd-darwin-arm64': 'https://example.com/argocd-darwin-arm64'});
        mockVersion('v3.4.1');
        render(<Help />);
        ...
        await waitFor(() => expect(screen.queryByText('GitHub releases')).not.toBeInTheDocument());
    });
});
