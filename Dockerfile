...
# Force download of latest linters
RUN go get -u github.com/argoproj/argo-cd && \
    cd $GOPATH/src/github.com/argoproj/argo-cd && \
    git remote add alexec https://github.com/alexec/argo-cd && \
    git fetch alexec && \
    git checkout golangci-lint && \
    dep ensure && \
    golangci-lint run
