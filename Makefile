...
define run-in-dev-tool
    docker run --rm -it -u $(shell id -u) -e HOME=/home/user -v ${GOPATH}/pkg/mod:/go/pkg/mod:cached -v ${CURRENT_DIR}:/src/argo-cd:cached -w /src/argo-cd argocd-dev-tools bash -c "GOPATH=/go $(1)"
endef
