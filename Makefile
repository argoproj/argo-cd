...
test-coverage:
	...
	@if [ "$(COVERALLS_TOKEN)" != "" ] ; then goveralls -ignore '*.pb.go,*.pb.gw.go' -coverprofile=coverage.out -service=argo-ci -repotoken "$(COVERALLS_TOKEN)"; else echo 'No COVERALLS_TOKEN env var specified. Skipping submission to Coveralls.io'; fi
