...
func TestListMatrixGenerator(t *testing.T) {
	...
	Given(t).
		...
		When().
		Delete(false).Then().Expect(ApplicationsDoNotExist(expectedAppsNewNamespace))
}
...
func TestMatrixTerminalMatrixGeneratorSelector(t *testing.T) {
	...
	Given(t).
		...
		When().
		Delete(false).Then().Expect(ApplicationsDoNotExist(excludedApps)).Expect(ApplicationsDoNotExist(expectedApps))
}
