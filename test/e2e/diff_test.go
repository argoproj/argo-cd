...
func TestMultiAppDiff(t *testing.T) {
	...
	// Create first app
	ctx.Path("guestbook").Name("multi-app-diff1")
	appName1 := ctx.AppName()
	ctx.When().CreateApp()
	...
	diffOutput, err := fixture.RunCli("app", "diff", appName1, appName2, "--exit-code=false")
	require.NoError(t, err)
	assert.Contains(t, diffOutput, "=== Application: "+appName1)
	assert.Contains(t, diffOutput, "=== Application: "+appName2)

	// 2. Run diff with --all flag.
	diffAllOutput, err := fixture.RunCli("app", "diff", "--all", "--exit-code=false")
	require.NoError(t, err)
	assert.Contains(t, diffAllOutput, "=== Application: "+appName1)
	assert.Contains(t, diffAllOutput, "=== Application: "+appName2)

	// 3. Test exit code 1 when diffs exist.
	_, err = fixture.RunCli("app", "diff", appName1, appName2, "--exit-code=true")
	require.Error(t, err)

	// 4. Sync both apps to remove the diffs.
	ctx.Name("multi-app-diff1").When().Sync()
	ctx.Name("multi-app-diff2").When().Sync()

	// 5. Run diff again. Diffs should be empty, and exit code should be 0.
	diffOutputSynced, err := fixture.RunCli("app", "diff", appName1, appName2, "--exit-code=true")
	require.NoError(t, err)
