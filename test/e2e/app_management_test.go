...
func TestKnownTypesInCRDDiffing(t *testing.T) {
	...
	Given(t).
		...
		When().
		And(func() {
			errors.NewHandler(t).CheckForErr(fixture.SetResourceOverrides(map[string]ResourceOverride{
				"argoproj.io/Dummy": {
...
func TestNotPermittedResources(t *testing.T) {
	...
	defer func() {
		...
		errors.NewHandler(t).CheckForErr(fixture.KubeClientset.NetworkingV1().Ingresses(fixture.TestNamespace()).Delete(context.Background(), "sample-ingress", metav1.DeleteOptions{}))
	}()
...
func TestDisableManifestGeneration(t *testing.T) {
	Given(t).
		...
		When().
		And(func() {
			errors.NewHandler(t).CheckForErr(fixture.SetEnableManifestGeneration(map[ApplicationSourceType]bool{
...
func TestAnnotationTrackingExtraResources(t *testing.T) {
	...
	errors.NewHandler(t).CheckForErr(fixture.SetTrackingMethod(string(argo.TrackingMethodAnnotation)))
	ctx.
		Path("deployment").
		When().
		CreateApp().
