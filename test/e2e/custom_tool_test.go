...
// A plugin whose discovery check never completes must not make the application fall back to the
// native generators: https://github.com/argoproj/argo-cd/issues/24004. The application controller's
// repo-server deadline is shortened so that the cmp-server kills the blocked discover command at its
// buffered deadline and reports that the check did not complete, instead of an answer.
func TestCMPDiscoveryNotCompletedDoesNotFallBack(t *testing.T) {
	if fixture.IsRemote() {
		t.Skip("The test restarts the application controller with goreman, which only works when not in cluster")
	}

	ctx := Given(t)

	// Given() has already restored the clean state, so the environment set here survives until the
	// next test's EnsureCleanState restarts the controller without it.
	err := fixture.RestartProcess(fixture.ApplicationControllerProcName, map[string]string{
		"ARGOCD_APPLICATION_CONTROLLER_REPO_SERVER_TIMEOUT_SECONDS": "5",
	})
	require.NoError(t, err)

	// The message discovery.Discover wraps an incomplete check in. It reaches the application as
	// part of the ComparisonError condition the controller records for a failed target state.
	const discoveryFailed = "failed to check for config management plugins"

	notFallenBack := func(app *Application) {
		assert.NotEqual(t, ApplicationSourceTypeDirectory, app.Status.SourceType, "the discovery check did not complete, so no native generator may have claimed the repository")
		assert.NotEqual(t, ApplicationSourceTypeHelm, app.Status.SourceType)
		assert.NotEqual(t, ApplicationSourceTypeKustomize, app.Status.SourceType)
		assert.Empty(t, app.Status.Resources, "manifest generation failed, so no resources may be reported")
	}

	ctx.
		RunningCMPServer("./testdata/cmp-discovery-not-completed").
		// The path holds plain YAML, so a fallback to the native generators would produce a
		// Directory application with resources.
		Path("cmp-discovery-not-completed").
		// The controller reports the failure one repo-server deadline after it starts comparing.
		Timeout(60).
		When().
		// Validation would run the same blocked discovery check through the API server, whose own
		// repo-server timeout this test cannot shorten.
		CreateApp("--validate=false").
		Then().
		Expect(Condition(ApplicationConditionComparisonError, discoveryFailed)).
		And(notFallenBack).
		// The second pass reads the repo-server manifest cache. It must report the same failure:
		// nothing generated by a native generator may have been cached against the revision.
		When().
