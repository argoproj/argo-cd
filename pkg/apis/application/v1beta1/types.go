...
// +kubebuilder:validation:XValidation:rule="!has(self.syncPolicy) || !has(self.syncPolicy.retry) || !has(self.syncPolicy.retry.limit) || self.syncPolicy.retry.limit >= -1",message="retry limit must be >= -1 (-1 is unlimited)"
