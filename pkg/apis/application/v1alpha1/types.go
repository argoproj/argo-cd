...
type ApplicationSourceHelm struct {
	...
	// IgnoreMissingFileParameters prevents helm template from failing when fileParameters do not exist locally by not appending them to helm template --set-file
	IgnoreMissingFileParameters bool `json:"ignoreMissingFileParameters,omitempty" protobuf:"bytes,16,opt,name=ignoreMissingFileParameters"`
}
...
func (ash *ApplicationSourceHelm) IsZero() bool {
	return ash == nil || (ash.Version == "") && (ash.ReleaseName == "") && len(ash.ValueFiles) == 0 && len(ash.Parameters) == 0 && len(ash.FileParameters) == 0 && ash.ValuesIsEmpty() && !ash.PassCredentials && !ash.IgnoreMissingValueFiles && !ash.SkipCrds && !ash.SkipTests && !ash.SkipSchemaValidation && ash.KubeVersion == "" && len(ash.APIVersions) == 0 && ash.Namespace == "" && !ash.IgnoreMissingFileParameters
}
