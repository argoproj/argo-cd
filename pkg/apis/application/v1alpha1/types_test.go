...
func TestAppProject_IsAppOfAppsPermitted(t *testing.T) {
	...
	var err error
	var allowed bool
	_, allowed, err = projNoRules.IsAppOfAppsPermitted(applicationSameProject)
	assert.NoError(t, err)
	assert.True(t, allowed)
	_, allowed, err = projNoRules.IsAppOfAppsPermitted(applicationDifferentProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projNoRules.IsAppOfAppsPermitted(applicationBlankProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projNoRules.IsAppOfAppsPermitted(applicationNoProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projNoRules.IsAppOfAppsPermitted(notAnApplication)
	assert.True(t, allowed)
	assert.NoError(t, err)

	_, allowed, err = projEmptyRules.IsAppOfAppsPermitted(applicationSameProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projEmptyRules.IsAppOfAppsPermitted(applicationDifferentProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projEmptyRules.IsAppOfAppsPermitted(applicationBlankProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projEmptyRules.IsAppOfAppsPermitted(applicationNoProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projEmptyRules.IsAppOfAppsPermitted(notAnApplication)
	assert.True(t, allowed)
	assert.NoError(t, err)

	_, allowed, err = projSameProjectOnlyFalse.IsAppOfAppsPermitted(applicationSameProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projSameProjectOnlyFalse.IsAppOfAppsPermitted(applicationDifferentProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projSameProjectOnlyFalse.IsAppOfAppsPermitted(applicationBlankProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projSameProjectOnlyFalse.IsAppOfAppsPermitted(applicationNoProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projSameProjectOnlyFalse.IsAppOfAppsPermitted(notAnApplication)
	assert.True(t, allowed)
	assert.NoError(t, err)

	_, allowed, err = projSameProjectOnlyTrue.IsAppOfAppsPermitted(applicationSameProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projSameProjectOnlyTrue.IsAppOfAppsPermitted(applicationDifferentProject)
	assert.False(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projSameProjectOnlyTrue.IsAppOfAppsPermitted(applicationBlankProject)
	assert.False(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projSameProjectOnlyTrue.IsAppOfAppsPermitted(applicationNoProject)
	assert.False(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = projSameProjectOnlyTrue.IsAppOfAppsPermitted(notAnApplication)
	assert.True(t, allowed)
	assert.NoError(t, err)

	_, allowed, err = defaultProjSameProjectOnlyTrue.IsAppOfAppsPermitted(applicationSameProject)
	assert.False(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = defaultProjSameProjectOnlyTrue.IsAppOfAppsPermitted(applicationDifferentProject)
	assert.False(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = defaultProjSameProjectOnlyTrue.IsAppOfAppsPermitted(applicationBlankProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = defaultProjSameProjectOnlyTrue.IsAppOfAppsPermitted(applicationNoProject)
	assert.True(t, allowed)
	assert.NoError(t, err)
	_, allowed, err = defaultProjSameProjectOnlyTrue.IsAppOfAppsPermitted(notAnApplication)
	assert.True(t, allowed)
	assert.NoError(t, err)
}
