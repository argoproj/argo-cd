...
func TestNullOrEmptyDoesNothing(t *testing.T) {
	...
	gitClient.EXPECT().RepoURL().Return("https://github.com/argoproj/argo-cd")

	t.Run("nil", func(t *testing.T) {
		var si *SourceIntegrity

		result, err := si.VerifyGit(gitClient, "")
	...
	t.Run("No matching policy", func(t *testing.T) {
		...
		result, err := si.VerifyGit(gitClient, "")
		require.NoError(t, err)
		assert.Equal(t, &SourceIntegrityCheckResult{}, result)
	})

	t.Run("Matching policy does nothing", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, &SourceIntegrityCheckResult{}, result)
		assert.Equal(t, []string{"No verification configured for SourceIntegrity policy for [*]"}, logger.GetEntries())
	})
}
...
func TestGPGHeadValid(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run("Verify "+test.revision, func(t *testing.T) {
			...
			gitClient.EXPECT().LsSignatures(mock.Anything, mock.Anything).RunAndReturn(func(revision string, deep bool) ([]git.RevisionSignatureInfo, error) {
				return []git.RevisionSignatureInfo{{
					revision, git.GPGVerificationResultGood, "KEYID", "ignored", "ignored",
				}}, nil
			})
			...
			gitClient.AssertCalled(t, "LsSignatures", test.revision, false)
			gitClient.AssertNotCalled(t, "IsAnnotatedTag", mock.Anything)
		...
		t.Run("Verify "+test.revision, func(t *testing.T) {
			...
			gitClient.EXPECT().TagSignature(mock.Anything).RunAndReturn(func(tagRevision string) (*git.RevisionSignatureInfo, error) {
				...
				case tagThird:
					keyId = "KEY_OF_THIRD"
				default:
					require.Fail(t, "tag revision '%s' not recognized", tagRevision)
				}
				return &git.RevisionSignatureInfo{
...
// TODO Test the destinction between unsigned commit vs. unsigned tag for strict mode
